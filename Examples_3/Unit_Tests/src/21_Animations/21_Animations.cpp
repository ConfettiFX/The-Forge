...
ThreadData gThreadData[MAX_ANIMATED_OBJECTS];
...
ThreadSkeletonData gThreadSkeletonData[MAX_ANIMATED_OBJECTS];
...
void SetUpAnimationSpecificGuiWindows()
{
	...
	static const uint32_t maxWidgetCount = max((uint32_t)GENERAL_PARAM_COUNT, max((uint32_t)IK_PARAM_COUNT, max((uint32_t)THREADING_PARAM_COUNT, (uint32_t)CLIP_PARAM_COUNT) ) );
...

	{
		...
		luaRegisterWidget(uiCreateComponentWidget(pStandaloneAnimationsGUIWindow, "General Settings", &CollapsingGeneralSettingsWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(pStandaloneAnimationsGUIWindow, "Threading Control", &CollapsingThreadingControlWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(pStandaloneAnimationsGUIWindow, "Inverse Kinematics", &CollapsingIKWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
	}
	...
	//
	{
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[0], "Stand Clip", &CollapsingStandClipWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
	}
	...
	//
	{
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[1], "Blend Parameters", &CollapsingBlendParamsWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[1], "Walk Clip", &CollapsingWalkClipWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[1], "Jog Clip", &CollapsingJogClipWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[1], "Run Clip", &CollapsingRunClipWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
	}
	...
	//
	{
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[2], "Blend Parameters", &CollapsingBlendParamsWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[2], "Upper Body Root", &CollapsingUpperBodyRootWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[2], "StandClip (UpperBody)", &CollapsingStandClipWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[2], "Walk Clip (Lower Body)", &CollapsingWalkClipWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
	}
	...
	//
	{
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[3], "Blend Parameters", &CollapsingBlendParamsWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[3], "Upper Body Masking", &CollapsingUpperBodyMaskWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[3], "Walk Clip", &CollapsingWalkClipWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
		...
		luaRegisterWidget(uiCreateComponentWidget(AnimationControlsGUIWindow[3], "NeckCrack Clip (Additive)", &CollapsingNeckCrackClipWidgets, WIDGET_TYPE_COLLAPSING_HEADER));
	}
...
public:
	bool Init()
	{
		...
		gOzzLogoAnimObject.mUpdateBones = false;
		...
		gJogClip.Initialize(RD_ANIMATIONS, gJogClipName, NULL, &gStickFigureRig);
		gRunClip.Initialize(RD_ANIMATIONS, gRunClipName, NULL, &gStickFigureRig);
		...
		for (unsigned int i = 0; i < MAX_ANIMATED_OBJECTS; i++)
		{
			...
			if (i % 2 == 1)
			{
				gStickFigureAnimObject[i].mJointColor = vec4(.1f, .9f, .1f, 1.f);
		...
		ozzSkeletonRenderDesc.mSkeletonPipeline = pOzzLogoSkeletonPipeline;
	...
	void Update(float deltaTime)
	{
		...
		if (gEnableThreading)
		{
			...
			addThreadSystemRangeTask(pThreadSystem, &Animations::AnimatedObjectThreadedUpdate, gThreadData, taskCount);
			...
			if (remainder != 0)
			{
				...
				addThreadSystemTask(pThreadSystem, &Animations::AnimatedObjectThreadedUpdate, &gThreadData[taskCount]);
			}
		}
		else
		{
		...
		if (gUIData.mGeneralSettings.mDrawBakedPhysics)
		{
			...
			gOzzLogoSkeletonBatcher.SetSharedUniforms(projViewMat, lightPos, lightColor);
		}
		else
		{
			pCameraController->update(deltaTime);
	...
	void Draw()
	{
		...
		if (gEnableThreading)
		{
			...
			addThreadSystemRangeTask(pThreadSystem, &Animations::SkeletonBatchUniformsThreaded, gThreadSkeletonData, taskCount);
			...
			if (remainder != 0)
			{
				...
				addThreadSystemTask(pThreadSystem, &Animations::SkeletonBatchUniformsThreaded, &gThreadSkeletonData[taskCount]);
			}
		...
		*(UniformSkeletonBlock*)targetViewProjCbv.pMappedData = gUniformDataTarget;
		...
		if (gUIData.mGeneralSettings.mDrawPlane)
		{
			...
			cmdBindDescriptorSet(cmd, gFrameIndex * 3 + 0, pDescriptorSet);
		...
		cmdBindDescriptorSet(cmd, gFrameIndex * 3 + 2, pDescriptorSet);
		...
		if (gUIData.mGeneralSettings.mDrawAttachedObject)
		{
			...
			cmdBindDescriptorSet(cmd, gFrameIndex * 3 + 1, pDescriptorSet);
	...
	const char* GetName() { return "21_Animations"; }
	...
	void addDescriptorSets()
	{
		...
		DescriptorSetDesc setDesc = { pRootSignature, DESCRIPTOR_UPDATE_FREQ_PER_DRAW, gImageCount * 3 };
		addDescriptorSet(pRenderer, &setDesc, &pDescriptorSet);

		gOzzLogoSkeletonBatcher.AddDescriptorSets(pRootSignature);
	...
	void removeDescriptorSets()
	{
		...
		gOzzLogoSkeletonBatcher.RemoveDescriptorSets();
	...
	void addPipelines()
	{
		...
		gOzzLogoSkeletonBatcher.LoadPipeline(pSkeletonPipeline);
	...
	{
		gOzzLogoSkeletonBatcher.PrepareDescriptorSets();
		...
		for (uint32_t i = 0; i < gImageCount; ++i)
		{
			...
			updateDescriptorSet(pRenderer, i * 3 + 0, pDescriptorSet, 1, params);
			params[0].pName = "uniformBlock";
			params[0].ppBuffers = &pCuboidUniformBuffer[i];
			updateDescriptorSet(pRenderer, i * 3 + 1, pDescriptorSet, 1, params);
			params[0].pName = "uniformBlock";
			params[0].ppBuffers = &pTargetUniformBuffer[i];
			updateDescriptorSet(pRenderer, i * 3 + 2, pDescriptorSet, 1, params);
		}
	}
