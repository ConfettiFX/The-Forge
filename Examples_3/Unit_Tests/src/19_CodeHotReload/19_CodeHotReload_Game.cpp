...
#endif
int tfMainCodeReload(struct cr_plugin* ctx, enum cr_op operation)
{
	switch (operation)
	{
	case CR_LOAD:   return onCodeLoad(ctx); // loading back from a reload
	case CR_UNLOAD: return onCodeUnload(ctx); // preparing to a new reload
	case CR_CLOSE: return onCodeClose(ctx); // the plugin will close and not reload anymore
	case CR_STEP: return onCodeUpdate(ctx);
	}
