...
#include "PerFrameSet.h"
	BEGIN_SRT_SET(PerBatch)
		DECL_RWBUFFER(PerBatch, RWBuffer(uint), gIndirectDrawFilteringArgs)
		DECL_RWBUFFER(PerBatch, RWBuffer(uint), gIndirectDataBufferRW)
		DECL_RWBUFFER(PerBatch, RWBuffer(uint), gIndirectDrawClearArgsRW)
		DECL_ARRAY_RWBUFFERS(PerBatch, RWByteBuffer, gFilteredIndexBufferRW, 10) /// NUM_CULLING_VIEWPORTS
