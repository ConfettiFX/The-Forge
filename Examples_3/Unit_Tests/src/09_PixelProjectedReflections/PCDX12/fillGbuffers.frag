...
{
    float3 tangentNormal =  normalMap.Sample(defaultSampler, uv).xyz * 2.0 - 1.0;
...
{	
	...
	//cut off
	float alpha = albedoMap.Sample(defaultSampler,input.uv).a;	
	...
	if(pbrMaterials!=-1) {
		...
		albedo = pow(albedoMap.Sample(defaultSampler,input.uv).rgb, float3(2.2, 2.2, 2.2));
		_metalness   = metallicMap.Sample(defaultSampler,input.uv).r;
		_roughness = roughnessMap.Sample(defaultSampler,input.uv).r;
		ao = aoMap.SampleLevel(defaultSampler,input.uv, 0).r;
	} 
