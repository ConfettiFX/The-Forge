...
layout (UPDATE_FREQ_NONE, binding=7) uniform sampler    uSampler1;
...
void main(void)
{
  ...
  if(side==1)
  {
      ...
      fs_out_color  =  texture(sampler2D(RightText, uSampler1), newtextcoord);
  }
  else if(side==2)
  {
      ...
      fs_out_color  =  texture(sampler2D(LeftText, uSampler1), newtextcoord);
  }
  else if(side==3)
  {
       fs_out_color  =  texture(sampler2D(TopText, uSampler1), (fs_in.texcoord.xz)/20+vec2(0.5));
  }
  else if(side == 4.0f)
  {
       ...
       newtextcoord = vec2(newtextcoord.x,1-newtextcoord.y);
       fs_out_color  =  texture(sampler2D(BotText, uSampler1), newtextcoord);
  }
  else if(side==5)
  {
       ...
       newtextcoord = vec2(newtextcoord.x,1-newtextcoord.y);
       fs_out_color = texture(sampler2D(FrontText, uSampler1), newtextcoord);
  ...
  else if(side==6)
  {
       ...
       newtextcoord = (fs_in.texcoord.xy)/20+vec2(0.5);
       newtextcoord = vec2(1-newtextcoord.x,1-newtextcoord.y);
       fs_out_color = texture(sampler2D(BackText, uSampler1), newtextcoord);
  }
