...
STRUCT(PlanetGroupData)
{
    DATA(float3, mPosition, None);
    DATA(float, mColorTint, None);
};
...
BEGIN_SRT_NO_AB(SrtData)
    ...
    BEGIN_SRT_SET(PerBatch)
        DECL_CBUFFER(PerBatch, CBUFFER(UniformData), gUniformBlock)
    END_SRT_SET(PerBatch)
    BEGIN_SRT_SET(PerDraw)
        DECL_CBUFFER(PerDraw, CBUFFER(PlanetGroupData), gPlanetGroupData)
    END_SRT_SET(PerDraw)
END_SRT(SrtData)
