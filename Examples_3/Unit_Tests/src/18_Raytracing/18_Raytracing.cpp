...
// Shader
#ifdef DIRECT3D12
#include "Shaders/D3D12/Compiled/RayGen.h"
#include "Shaders/D3D12/Compiled/ClosestHit.h"
#include "Shaders/D3D12/Compiled/ClosestHitPlane.h"
#include "Shaders/D3D12/Compiled/ClosestHitShadow.h"
#include "Shaders/D3D12/Compiled/Miss.h"
#include "Shaders/D3D12/Compiled/MissShadow.h"
#endif
...
#ifdef _WINDOWS
	"../../../src/18_Raytracing/",       // FSR_BinShaders
	"../../../src/18_Raytracing/",       // FSR_SrcShaders
	"../../../UnitTestResources/",                  // FSR_Textures
	"../../../UnitTestResources/",                  // FSR_Meshes
	"../../../UnitTestResources/",                  // FSR_Builtin_Fonts
	"../../../src/18_Raytracing/",       // FSR_GpuConfig
	"",                                             // FSR_Animation
	"",                                             // FSR_OtherFiles
...
public:
	...
	bool Init()
	{
		...
		addRaytracingRootSignature(pRaytracing, nullptr, 0, true, &pEmptyRootSignature);
        /************************************************************************/
        // 03 - Create Raytracing Pipeline
        /************************************************************************/
        const char* pNames[] = { "rayGen", "miss", "chs", "chsPlane", "chsShadow", "missShadow" };
        
        //Rustam: unify shaders loading
#ifdef DIRECT3D12
		addRaytracingShader(pRaytracing, gShader_RayGen, sizeof(gShader_RayGen), pNames[0], &pShaderRayGen);
		addRaytracingShader(pRaytracing, gShader_ClosestHit, sizeof(gShader_ClosestHit), pNames[2], &pShaderHitTriangle);
...
#else
        addRaytracingShader(pRaytracing, (const unsigned char*)"RayGen.comp", 0, pNames[0], &pShaderRayGen);
        addRaytracingShader(pRaytracing, (const unsigned char*)"ClosestHit.comp", 0, pNames[2], &pShaderHitTriangle);
        addRaytracingShader(pRaytracing, (const unsigned char*)"ClosestHitPlane.comp", 0, pNames[3], &pShaderHitPlane);
        addRaytracingShader(pRaytracing, (const unsigned char*)"ClosestHitShadow.comp", 0, pNames[4], &pShaderHitShadow);
        addRaytracingShader(pRaytracing, (const unsigned char*)"Miss.comp", 0, pNames[1], &pShaderMiss);
        addRaytracingShader(pRaytracing, (const unsigned char*)"MissShadow.comp", 0, pNames[5], &pShaderMissShadow);
#endif
        ...
        /************************************************************************/
        {
            ...
            shaderResources[0].used_stages = SHADER_STAGE_COMP;
            addRaytracingRootSignature(pRaytracing, shaderResources, 1, true, &pRayGenSignature);
            ...
            addRaytracingRootSignature(pRaytracing, shaderResources, 1, true, &pPlaneHitSignature);
        }
        /************************************************************************/
        
        RaytracingHitGroup hitGroups[3] = {};
        hitGroups[0].pClosestHitShader    = pShaderHitTriangle; //"chs"
        hitGroups[0].pHitGroupName        = "hitGroup";
        
        hitGroups[1].pClosestHitShader    = pShaderHitPlane; //"chsPlane"
        hitGroups[1].pHitGroupName        = "hitGroupPlane";
        hitGroups[1].pRootSignature        = pPlaneHitSignature;
        
        hitGroups[2].pClosestHitShader    = pShaderHitShadow; //"chsShadow"
	...
	void Exit()
	{
		...
		if (pRaytracing != nullptr)
		{
			...
			removeRaytracingShader(pRaytracing, pShaderHitTriangle);
			removeRaytracingShader(pRaytracing, pShaderHitPlane);
			removeRaytracingShader(pRaytracing, pShaderMiss);
			removeRaytracingShader(pRaytracing, pShaderHitShadow);
			removeRaytracingShader(pRaytracing, pShaderMissShadow);
	...
	bool Load()
	{
        ...
        addPipeline(pRenderer, &pipelineSettings, &pDisplayTexturePipeline);
		/************************************************************************/
		/************************************************************************/
...
private:
	...
	RaytracingShader*	   pShaderHitTriangle;
