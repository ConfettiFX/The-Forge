...
public:
	...
	bool Init()
	{
		...
		//Gbuffer pass
		RenderPassData *pass = conf_placement_new<RenderPassData>(conf_calloc(1, sizeof(RenderPassData)), pRenderer, pGraphicsQueue, gImageCount);
	...
	bool LoadSponza()
	{
		...
		for (int i = 0; i < meshCount; i++)
		{
			...
			Mesh subMesh = gModel_Sponza.mMeshArray[i];
	...
	void Draw()
	{
		...
		// GPrepass *********************************************************************************
		{
			...
			cmdBindRenderTargets(cmd, (uint32_t)RenderPasses[RenderPass::GBuffer]->RenderTargets.size(), RenderPasses[RenderPass::GBuffer]->RenderTargets.data(), pDepthBuffer, &loadActions, NULL, NULL, -1, -1);
			cmdSetViewport(cmd, 0.0f, 0.0f, (float)RenderPasses[RenderPass::GBuffer]->RenderTargets[0]->mDesc.mWidth, (float)RenderPasses[RenderPass::GBuffer]->RenderTargets[0]->mDesc.mHeight, 0.0f, 1.0f);
			cmdSetScissor(cmd, 0, 0, RenderPasses[RenderPass::GBuffer]->RenderTargets[0]->mDesc.mWidth, RenderPasses[RenderPass::GBuffer]->RenderTargets[0]->mDesc.mHeight);
		...
		// Raytraced shadow pass ************************************************************************
		{
			...
			TextureBarrier barriers[] = {
				...
				{ RenderPasses[RenderPass::GBuffer]->RenderTargets[GBufferRT::Normals]->pTexture, RESOURCE_STATE_SHADER_RESOURCE },
		...
		// Lighting pass *********************************************************************************
		{
			...
			TextureBarrier barriers[] = {
				{ RenderPasses[RenderPass::RaytracedShadows]->Textures[0], RESOURCE_STATE_SHADER_RESOURCE },
