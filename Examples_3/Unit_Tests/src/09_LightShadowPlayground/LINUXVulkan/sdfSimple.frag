...
float closestDistLocal(vec3 pos) 
{
	...
	float result = min(sdPlaneLocal(pos.xyz - vec3(0, 0.0f, 0)),//plane transformation
	  	sdSphereLocal(pos.xyz - vec3(0, 0.29f, 0.0f), sphereRadius));//sphere translation and scale
	  
	for (int i = 0; i < SPHERE_EACH_ROW; i++){
	  for (int j = 0; j < SPHERE_EACH_COL; j++){
	    result = min(result, sdSphereLocal(pos.xyz - vec3(sphereDist*i, 0.29f, sphereDist*j), sphereRadius));
	  }
	}
	return result;
}
