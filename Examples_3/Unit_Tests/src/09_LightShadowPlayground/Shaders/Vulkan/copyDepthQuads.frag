...
layout(set = 0, binding = 2) uniform texture2D DepthPassTexture;
float HLSLmain(VSOutput input1)
{
    float tileDepth = float ((textureLod(sampler2D( DepthPassTexture, clampToEdgeNearSampler), vec2((input1).UV), float (0.0)) + vec4 (((input1).MiscData).z)));
    return max(float (0.0), max(float (0.0), tileDepth));
}
void main()
{
    VSOutput input1;
