...
struct Fragment_Shader
{
    ...
    {
        float tileDepth = (float)(DepthPassTexture.sample(clampToEdgeNearSampler, (input).UV, level(0.0)) + (float4)(((input).MiscData).z)).r;
        return max((float)0.0,(float)max((float)0.0,(float)tileDepth));
    };
