...
#include "PerFrameSet.h"
    BEGIN_SRT_SET(PerDraw)
        ...
        DECL_TEXTURE(PerDraw, Tex2D(float4), gAlbedoMap)
        DECL_TEXTURE(PerDraw, Tex2D(float4), gNormalMap)
        DECL_TEXTURE(PerDraw, Tex2D(float4), gMetallicMap)
        DECL_TEXTURE(PerDraw, Tex2D(float4), gRoughnessMap)
        DECL_TEXTURE(PerDraw, Tex2D(float4), gAOMap)
        DECL_TEXTURE(PerDraw, Tex2D(float4), gVMFMap)
     END_SRT_SET(PerDraw)
END_SRT(SrtData)
