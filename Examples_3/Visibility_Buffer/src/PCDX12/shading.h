...
static const int NUM_SHADOW_SAMPLES = 32;
...
float3 calculateIllumination(float3 normal, float3 camPos, float esmControl, float3 normalizedDirToLight, bool isTwoSided, float4 posLS, float3 position, Texture2D shadowMap, float3 albedo, float3 specularColor, float ao, SamplerState sh)
{
	...
	if (all(posLS.xy > 0) && all(posLS.xy < 1))
	{
		...
		shadowDepthSample.x = shadowMap.SampleLevel(sh, posLS.xy, 0).r;
		shadowDepthSample.y = shadowMap.SampleLevel(sh, posLS.xy, 0, int2(1, 0)).r;
		shadowDepthSample.z = shadowMap.SampleLevel(sh, posLS.xy, 0, int2(0, 1)).r;
		shadowDepthSample.w = shadowMap.SampleLevel(sh, posLS.xy, 0, int2(1, 1)).r;
