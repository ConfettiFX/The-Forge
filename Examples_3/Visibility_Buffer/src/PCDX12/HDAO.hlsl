...
#if SAMPLE_COUNT > 1
    uint3 loc = uint3(f2TexCoord * g_f2RTSize, 0);
    f4Ret.x = Tex.Load(loc, 0, int2(0,1));
    f4Ret.y = Tex.Load(loc, 0, int2(1,1));
    f4Ret.z = Tex.Load(loc, 0, int2(1,0));
    f4Ret.w = Tex.Load(loc, 0, int2(0,0));
...
float PSMain(PsIn In) : SV_Target
{
    ...
    {                
        ...
        float fDepth = 0;
#if SAMPLE_COUNT > 1
        fDepth = g_txDepth.Load(uint3(ui2ScreenCoord, 0), 0);
#else
