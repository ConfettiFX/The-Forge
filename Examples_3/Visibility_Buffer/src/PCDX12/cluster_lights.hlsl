...
RWStructuredBuffer<uint> lightClustersCount : register(u0);
RWStructuredBuffer<uint> lightClusters : register(u1);
ConstantBuffer<PerFrameConstants> uniforms : register(b2);
StructuredBuffer<LightData> lights : register(t3);

[numthreads(LIGHT_CLUSTER_WIDTH, LIGHT_CLUSTER_HEIGHT, 1)]
void CSMain(uint3 threadInGroupId : SV_GroupThreadID, uint3 groupId : SV_GroupID)
{
