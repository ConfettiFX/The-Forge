...
StructuredBuffer<uint> vertexAttr;
...
float4 PSMain(VSOutput input, uint i : SV_SampleIndex) : SV_Target
{
	...
	if (alphaBit_drawID_triID != ~0)
	{
		...
		float3x2 texCoords =
		{
			unpack2Floats(asuint(vertexAttr.Load(index0 * 3))) * one_over_w[0],
			unpack2Floats(asuint(vertexAttr.Load(index1 * 3))) * one_over_w[1],
			unpack2Floats(asuint(vertexAttr.Load(index2 * 3))) * one_over_w[2]
		};
		...
		float3x3 normals =
		{
			decodeDir(unpackUnorm2x16(asuint(vertexAttr.Load(index0 * 3 + 1)))) * one_over_w[0],
			decodeDir(unpackUnorm2x16(asuint(vertexAttr.Load(index1 * 3 + 1)))) * one_over_w[1],
			decodeDir(unpackUnorm2x16(asuint(vertexAttr.Load(index2 * 3 + 1)))) * one_over_w[2]
		};
		...
		float3x3 tangents =
		{
			decodeDir(unpackUnorm2x16(asuint(vertexAttr.Load(index0 * 3 + 2)))) * one_over_w[0],
			decodeDir(unpackUnorm2x16(asuint(vertexAttr.Load(index1 * 3 + 2)))) * one_over_w[1],
			decodeDir(unpackUnorm2x16(asuint(vertexAttr.Load(index2 * 3 + 2)))) * one_over_w[2]
		};
		...
		// directional light
		shadedColor = calculateIllumination(normal, uniforms.camPos.xyz, uniforms.esmControl, uniforms.lightDir.xyz, isTwoSided, posLS, position, shadowMap, diffuseColor.xyz, specularData.xyz, ao, depthSampler);
