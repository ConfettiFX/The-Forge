...
vertex PsIn VSMain(uint vertexID [[vertex_id]])
{
...
fragment float4 PSMain(PsIn In [[stage_in]],
                      ...
                      sampler g_SamplePoint                         [[sampler(1)]],
#if SAMPLE_COUNT > 1
                      depth2d_ms<float,access::read>  g_txDepth      [[texture(2)]])
#else
                      depth2d<float,access::sample>  g_txDepth      [[texture(2)]])
#endif
{
    // Locals
