...
kernel void stageMain(uint3 inGroupId [[thread_position_in_threadgroup]],
					  ...
					  constant PerFrameConstants& uniforms                    [[buffer(UNIT_UNIFORMS_CBV)]],
...
)
{
	...
	uint batchMeshIndex = batchData[groupId.x].meshIndex;
	uint batchInputIndexOffset = (meshConstantsBuffer[batchMeshIndex].indexOffset + batchData[groupId.x].indexOffset);
	bool twoSided = (meshConstantsBuffer[batchMeshIndex].twoSided == 1);

	uint indices[3] = { 0, 0, 0 };
	if (inGroupId.x < batchData[groupId.x].faceCount)
	{
	...
	uint accumBatchDrawIndex = batchData[groupId.x].accumDrawIndex;
	...
	if (inGroupId.x == 0)
	{
		for (uint i = 0; i < NUM_CULLING_VIEWPORTS; ++i)
		{
			...
			atomic_store_explicit(&workGroupOutputSlot[i],
								  atomic_fetch_add_explicit((device atomic_uint*)&uncompactedDrawArgsRW.data[i][accumBatchDrawIndex].numIndices, index, memory_order_relaxed),
	...
	for (uint i = 0; i < NUM_CULLING_VIEWPORTS; ++i)
	{
		if (!cull[i])
		{
			...
			filteredIndicesBuffer.data[i][index + batchData[groupId.x].outputIndexOffset + threadOutputSlot[i] + 0] = indices[0];
			filteredIndicesBuffer.data[i][index + batchData[groupId.x].outputIndexOffset + threadOutputSlot[i] + 1] = indices[1];
			filteredIndicesBuffer.data[i][index + batchData[groupId.x].outputIndexOffset + threadOutputSlot[i] + 2] = indices[2];
		}
	}
	
	if (inGroupId.x == 0 && groupId.x == batchData[groupId.x].drawBatchStart)
	{
		uint outIndexOffset = batchData[groupId.x].outputIndexOffset;
		
		for (uint i = 0; i < NUM_CULLING_VIEWPORTS; ++i)
		{
			uncompactedDrawArgsRW.data[i][accumBatchDrawIndex].startIndex = outIndexOffset;
			uncompactedDrawArgsRW.data[i][accumBatchDrawIndex].materialID = meshConstantsBuffer[batchMeshIndex].materialID;
		}
	}
}
