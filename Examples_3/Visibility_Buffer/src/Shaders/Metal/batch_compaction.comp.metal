...
)
{
    ...
    {
        numIndices[i] = csDataPerFrame.uncompactedDrawArgs[i][groupId].numIndices;
        sum += numIndices[i];
    }
    ...
    for (uint i = 0; i < NUM_CULLING_VIEWPORTS; ++i)
    {
        if (numIndices[i] > 0)
        {
            uint matID = csDataPerFrame.uncompactedDrawArgs[i][groupId].materialID;
            ...
            if (hasAlpha)
            {
                ...
                atomic_store_explicit(&csDataPerFrame.indirectDrawArgsBufferAlpha[i][slot * INDIRECT_DRAW_ARGUMENTS_STRUCT_NUM_ELEMENTS + 2], csDataPerFrame.uncompactedDrawArgs[i][groupId].startIndex, memory_order_relaxed);
            }
            else
            {
                ...
                atomic_store_explicit(&csDataPerFrame.indirectDrawArgsBufferNoAlpha[i][slot * INDIRECT_DRAW_ARGUMENTS_STRUCT_NUM_ELEMENTS + 2], csDataPerFrame.uncompactedDrawArgs[i][groupId].startIndex, memory_order_relaxed);
            }
