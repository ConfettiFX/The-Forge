...
float3 calculateSpecular(float3 specularColor, float3 pCamPos, float3 pixelPos, float3 normalizedDirToLight, float3 normal)
{
	float3 viewVec = normalize(pCamPos - pixelPos);
	...
	inout(float) shadowFactor)
{
	...
	if( all(GreaterThan(posLS.xy, 0)) && all(LessThan(posLS.xy, 1)))
	{
		...
		// ESM
		float CShadow = linearDepth(pShadowMap.SampleLevel(sh, posLS.xy, 0).r);

		float LShadow = linearDepth(pShadowMap.SampleLevel(sh, posLS.xy + float2(-Gaps.x, 0.0), 0).r);
		float RShadow = linearDepth(pShadowMap.SampleLevel(sh, posLS.xy + float2(Gaps.x, 0.0), 0).r);

		float LTShadow = linearDepth(pShadowMap.SampleLevel(sh, posLS.xy + float2(-Gaps.x, Gaps.y), 0).r);
		float RTShadow = linearDepth(pShadowMap.SampleLevel(sh, posLS.xy + float2(Gaps.x, Gaps.y), 0).r);

		float TShadow = linearDepth(pShadowMap.SampleLevel(sh, posLS.xy + float2(0.0, Gaps.y), 0).r);
		float BShadow = linearDepth(pShadowMap.SampleLevel(sh, posLS.xy + float2(0.0, -Gaps.y), 0).r);

		float LBShadow = linearDepth(pShadowMap.SampleLevel(sh, posLS.xy + float2(-Gaps.x, -Gaps.y), 0).r);
		float RBShadow = linearDepth(pShadowMap.SampleLevel(sh, posLS.xy + float2(Gaps.x, -Gaps.y), 0).r);
		...
		float esm = exp(-pEsmControl * linearD) * exp(pEsmControl * avgShadowDepthSample);
