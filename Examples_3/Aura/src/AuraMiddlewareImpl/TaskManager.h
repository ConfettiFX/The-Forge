...
public:
    ...
    //  than a task.
    bool CreateTaskSet(uint32_t group, TASKSETFUNC pFunc, /* Function pointer to the */ /* Taskset callback function */
                       void*          pArg,                                             /* App data pointer (can be NULL) */
                       uint32_t       uTaskCount,                                       /* Number of tasks to create */
                       TASKSETHANDLE* pDepends,
                       /* Array of TASKSETHANDLEs that */ /* this taskset depends on. The */ /* taskset will not be scheduled */
