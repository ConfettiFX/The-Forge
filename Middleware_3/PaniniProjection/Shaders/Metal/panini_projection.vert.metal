...
vertex VSOutput stageMain(VSInput input [[stage_in]],
                       constant PaniniParameters& PaniniRootConstants [[buffer(1)]])
{
    VSOutput output;
    
    // Geometry: Tessellated Quad
    output.position = input.position;                   // [-1, +1]
    output.texcoord = input.position.xy * 0.5 + 0.5;    // [ 0,  1]
    ...
    output.position.xy = PaniniProjectionScreenPosition(output.position.xy, fovH, D, S, upscale);
