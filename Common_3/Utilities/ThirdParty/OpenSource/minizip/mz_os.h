...
#include <dirent.h>
	...
	bool mz_path_resolve(const char* path, char* target, int32_t max_target);
	/* Resolves path */
	...
	//bool mz_file_get_crc(ResourceDirectory directory,  const char *path, const char* password, uint32_t *result_crc);
	...
	//
	//bool  mz_os_rand(uint8_t *buf, int32_t size);
	///* Random number generator (not cryptographically secure) */
	//
	//bool  mz_os_rename(const char *source_path, const char *target_path);
	///* Rename a file */
	//
	//bool  mz_os_unlink(const char *path);
	///* Delete an existing file  */
	//
	//bool  mz_os_file_exists(const char *path);
	///* Check to see if a file exists */
	//
	//int64_t  mz_os_get_file_size(const char *path);
	///* Gets the length of a file */
	//
	//bool  mz_os_get_file_date(const char *path, time_t *modified_date, time_t *accessed_date, time_t *creation_date);
	///* Gets a file's modified, access, and creation dates if supported */
	//
	//bool  mz_os_set_file_date(const char *path, time_t modified_date, time_t accessed_date, time_t creation_date);
	///* Sets a file's modified, access, and creation dates if supported */
	//
	//bool  mz_os_get_file_attribs(const char *path, uint32_t *attributes);
	///* Gets a file's attributes */
	//
	//bool  mz_os_set_file_attribs(const char *path, uint32_t attributes);
	///* Sets a file's attributes */

	//bool  mz_os_make_dir(const char *path);
	...
	//
	//bool  mz_os_close_dir(DIR *dir);
	///* Closes a directory that has been opened for listing */

	bool mz_os_is_dir(const char* path);
	/* Checks to see if path is a directory */

	//bool  mz_os_is_symlink(const char *path);
	///* Checks to see if path is a symbolic link */
	//
	//bool  mz_os_make_symlink(const char *path, const char *target_path);
	///* Creates a symbolic link pointing to a target */
	//
	//bool  mz_os_read_symlink(const char *path, char *target_path, int32_t max_target_path);
