...
int32_t mz_crypt_rand(uint8_t* buf, int32_t size) { return mz_os_rand(buf, size); }
#endif
...
bool mz_crypt_pbkdf2(
	uint8_t* password, size_t password_length, uint8_t* salt, size_t salt_length, size_t iteration_count, uint8_t* key, size_t key_length)
{
	void*    hmac1 = NULL;
	void*    hmac2 = NULL;
	void*    hmac3 = NULL;
	...
	noerr = mz_crypt_hmac_init(hmac1, password, password_length);
	if (noerr)
		noerr = mz_crypt_hmac_init(hmac2, password, password_length);
	if (noerr)
		noerr = mz_crypt_hmac_update(hmac2, salt, salt_length);

	block_count = 1 + ((uint16_t)key_length - 1) / MZ_HASH_SHA1_SIZE;

	for (i = 0; noerr && (i < block_count); i += 1)
	{
		memset(ux, 0, sizeof(ux));

		noerr = mz_crypt_hmac_copy(hmac2, hmac3);
		if (!noerr)
			break;
		...
		{
			noerr = mz_crypt_hmac_update(hmac3, uu, k);
			if (noerr)
				noerr = mz_crypt_hmac_end(hmac3, uu, sizeof(uu));
			if (!noerr)
				break;
			...
			noerr = mz_crypt_hmac_copy(hmac1, hmac3);
			if (!noerr)
				break;
		}
