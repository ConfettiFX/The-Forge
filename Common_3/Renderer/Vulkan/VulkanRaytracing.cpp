	...
	Raytracing* pRaytracing, const AccelerationStructureDescTop* pDesc, uint64_t* pScratchBufferSize, AccelerationStructure* pOut)
{
	...
	/************************************************************************/
	eastl::vector<VkAccelerationStructureInstanceKHR> instanceDescs(pDesc->mInstancesDescCount);
	for (uint32_t i = 0; i < pDesc->mInstancesDescCount; ++i)
	{
		...
		memcpy(&instanceDescs[i].transform, &transformMatrix, sizeof(VkTransformMatrixKHR));
	}
	...
	memcpy(pOut->pInstanceDescBuffer->pCpuMappedAddress, instanceDescs.data(), instanceDesc.mSize);
	...
	Raytracing* pRaytracing, ShaderLocalData* mShaderLocalData, const uint8_t* shaderHandleStorage)
{
	...
	for (uint32_t idx = 0; idx < shaderCount; ++idx)
	{
		...
		const char**  it = eastl::find(
			pTablePipeline->mVulkan.ppShaderStageNames, pTablePipeline->mVulkan.ppShaderStageNames + pTablePipeline->mVulkan.mShaderStageCount,
			nameStr.c_str(), [](const char* a, const char* b) { return strcmp(a, b) == 0; });
		if (it != pTablePipeline->mVulkan.ppShaderStageNames + pTablePipeline->mVulkan.mShaderStageCount)
		{
