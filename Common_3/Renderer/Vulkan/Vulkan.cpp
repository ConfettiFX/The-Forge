...
extern long vk_createBuffer(MemoryAllocator* pAllocator, const BufferCreateInfo* pCreateInfo, const AllocatorMemoryRequirements* pMemoryRequirements, Buffer* pBuffer);
extern void vk_destroyBuffer(MemoryAllocator* pAllocator, struct Buffer* pBuffer);
extern long vk_createTexture(MemoryAllocator* pAllocator, const TextureCreateInfo* pCreateInfo, const AllocatorMemoryRequirements* pMemoryRequirements, Texture* pTexture);
...
namespace vk {
	...
	void addQueue(Renderer* pRenderer, QueueDesc* pDesc, Queue** ppQueue)
	{
		...
		if (found)
		{
			...
			//get queue handle
			vkGetDeviceQueue(pRenderer->pVkDevice, pQueueToCreate->mVkQueueFamilyIndex, pQueueToCreate->mVkQueueIndex, &(pQueueToCreate->pVkQueue));
	...
	void addTexture(Renderer* pRenderer, const TextureDesc* pDesc, Texture** ppTexture)
	{
		...
		/************************************************************************/
		{
			...
			case TEXTURE_TYPE_CUBE:
				view_type = pDesc->mArraySize > 1 ? VK_IMAGE_VIEW_TYPE_CUBE_ARRAY : VK_IMAGE_VIEW_TYPE_CUBE;
				break;
			}
			...
			add_info.subresourceRange.baseMipLevel = pDesc->mBaseMipLevel;
			add_info.subresourceRange.levelCount = pDesc->mMipLevels;
			add_info.subresourceRange.baseArrayLayer = pDesc->mBaseArrayLayer;
			add_info.subresourceRange.layerCount = pDesc->mArraySize * (pDesc->mType == TEXTURE_TYPE_CUBE ? 6 : 1);
			VkResult vk_res = vkCreateImageView(pRenderer->pVkDevice, &add_info, NULL, &(pTexture->pVkImageView));
			ASSERT(VK_SUCCESS == vk_res);
		...
		if (additionalFlags != 0 && !(pDesc->mUsage & TEXTURE_USAGE_UNORDERED_ACCESS))
		{
		...
		else
		{
			...
			else if (pDesc->mUsage & TEXTURE_USAGE_SAMPLED_IMAGE)
	...
	void removeTexture(Renderer* pRenderer, Texture* pTexture)
	{
		...
		if (VK_NULL_HANDLE != pTexture->pVkImageView) {
			vkDestroyImageView(pRenderer->pVkDevice, pTexture->pVkImageView, NULL);
		}
	...
	void addRenderTarget(Renderer* pRenderer, const RenderTargetDesc* pDesc, RenderTarget** ppRenderTarget)
	{
		...
		if (pDesc->mUsage == RENDER_TARGET_USAGE_DEPTH_STENCIL)
		{
			...
			if (VK_FORMAT_UNDEFINED != vk_depth_stencil_format) {
				...
				VkResult vk_res = vkGetPhysicalDeviceImageFormatProperties(pRenderer->pVkActiveGPU,
		...
		for (uint32_t i = 0; i < SHADER_STAGE_COUNT; ++i) {
			...
			if (stage_mask == (pShaderProgram->mStages & stage_mask)) {
				...
				case SHADER_STAGE_VERT: {
					vk_createShaderReflection((const uint8_t*)pDesc->mVert.pByteCode, (uint32_t)pDesc->mVert.mByteCodeSize, SHADER_STAGE_VERT, &stageReflections[counter]);
				...
				case SHADER_STAGE_TESC: {
					vk_createShaderReflection((const uint8_t*)pDesc->mHull.pByteCode, (uint32_t)pDesc->mHull.mByteCodeSize, SHADER_STAGE_TESC, &stageReflections[counter]);
				...
				case SHADER_STAGE_TESE: {
					vk_createShaderReflection((const uint8_t*)pDesc->mDomain.pByteCode, (uint32_t)pDesc->mDomain.mByteCodeSize, SHADER_STAGE_TESE, &stageReflections[counter]);
				...
				case SHADER_STAGE_GEOM: {
					vk_createShaderReflection((const uint8_t*)pDesc->mGeom.pByteCode, (uint32_t)pDesc->mGeom.mByteCodeSize, SHADER_STAGE_GEOM, &stageReflections[counter]);
				...
				case SHADER_STAGE_FRAG: {
					vk_createShaderReflection((const uint8_t*)pDesc->mFrag.pByteCode, (uint32_t)pDesc->mFrag.mByteCodeSize, SHADER_STAGE_FRAG, &stageReflections[counter]);
				...
				case SHADER_STAGE_COMP: {
					vk_createShaderReflection((const uint8_t*)pDesc->mComp.pByteCode, (uint32_t)pDesc->mComp.mByteCodeSize, SHADER_STAGE_COMP, &stageReflections[counter]);
	...
	void acquireNextImage(Renderer* pRenderer, SwapChain* pSwapChain, Semaphore* pSignalSemaphore, Fence* pFence, uint32_t* pImageIndex)
	{
		...
		if (pFence != NULL)
		{
			vk_res = vkAcquireNextImageKHR(pRenderer->pVkDevice,
	...
	{
		vmaFreeStatsString(pRenderer->pVmaAllocator, stats);
	}
	/************************************************************************/
