...
#define VENDOR_ID_INTEL_2 0x8087
	...
	static void add_rtv(Renderer* pRenderer, ID3D11Resource* pResource, DXGI_FORMAT format, uint32_t mipSlice, uint32_t arraySlice, ID3D11RenderTargetView** pHandle)
	{
	...
	static void add_dsv(Renderer* pRenderer, ID3D11Resource* pResource, DXGI_FORMAT format, uint32_t mipSlice, uint32_t arraySlice, ID3D11DepthStencilView** pHandle)
	{
	...
	void cmdUpdateSubresources(Cmd* pCmd, uint32_t startSubresource, uint32_t numSubresources, SubresourceDataDesc* pSubresources, Buffer* pIntermediate, uint64_t intermediateOffset, Texture* pTexture)
	{
		...
		{
			cmd.mUpdateSubresourcesCmd.pSubresources[i].pData = conf_calloc(cmd.mUpdateSubresourcesCmd.pSubresources[i].mSlicePitch, sizeof(uint8_t));
	...
	void addSwapChain(Renderer* pRenderer, const SwapChainDesc* pDesc, SwapChain** ppSwapChain)
	{
...
#else
		HRESULT hres = pRenderer->pDXGIFactory->CreateSwapChain(
			pRenderer->pDxDevice,
			&desc, &swapchain);
		ASSERT(SUCCEEDED(hres));
		...
		pSwapChain->ppDxSwapChainResources = (ID3D11Resource**)conf_calloc(pSwapChain->mDesc.mImageCount, sizeof(*pSwapChain->ppDxSwapChainResources));
	...
	void addRenderTarget(Renderer* pRenderer, const RenderTargetDesc* pDesc, RenderTarget** ppRenderTarget)
	{
		...
		if (isDepth)
		{
			pRenderTarget->pDxDsvDescriptors =
				(ID3D11DepthStencilView**)conf_calloc(numRTVs + 1, sizeof(ID3D11DepthStencilView*));
		}
		else
		{
			pRenderTarget->pDxRtvDescriptors =
				(ID3D11RenderTargetView**)conf_calloc(numRTVs + 1, sizeof(ID3D11RenderTargetView*));
		}
				...
				(pDesc->mDescriptors & DESCRIPTOR_TYPE_RENDER_TARGET_DEPTH_SLICES))
			{
				for (uint32_t j = 0; j < depthOrArraySize; ++j)
				{
					!ImageFormat::IsDepthFormat(pRenderTarget->mDesc.mFormat) ?
						add_rtv(pRenderer, pRenderTarget->pTexture->pDxResource, dxFormat, i, j, &pRenderTarget->pDxRtvDescriptors[1 + i * depthOrArraySize + j]) :
						add_dsv(pRenderer, pRenderTarget->pTexture->pDxResource, dxFormat, i, j, &pRenderTarget->pDxDsvDescriptors[1 + i * depthOrArraySize + j]);
				}
			}
			else
			{
	...
	void addSampler(Renderer* pRenderer, const SamplerDesc* pDesc, Sampler** ppSampler)
	{
		...
		desc.Filter = util_to_dx_filter(pDesc->mMinFilter, pDesc->mMagFilter, pDesc->mMipMapMode,
	...
	void addShaderBinary(Renderer* pRenderer, const BinaryShaderDesc* pDesc, Shader** ppShaderProgram)
	{
		...
		for (uint32_t i = 0; i < SHADER_STAGE_COUNT; ++i)
		{
			...
			if (stage_mask == (pShaderProgram->mStages & stage_mask))
			{
				...
				case SHADER_STAGE_VERT: {
					pStage = &pDesc->mVert;
					HRESULT hr = pRenderer->pDxDevice->CreateVertexShader(pStage->pByteCode, pStage->mByteCodeSize, NULL, &pShaderProgram->pDxVertexShader);
					ASSERT(SUCCEEDED(hr));
				...
				case SHADER_STAGE_HULL: {
					pStage = &pDesc->mHull;
					HRESULT hr = pRenderer->pDxDevice->CreateHullShader(pStage->pByteCode, pStage->mByteCodeSize, NULL, &pShaderProgram->pDxHullShader);
					ASSERT(SUCCEEDED(hr));
				} break;
				case SHADER_STAGE_DOMN: {
					pStage = &pDesc->mDomain;
					HRESULT hr = pRenderer->pDxDevice->CreateDomainShader(pStage->pByteCode, pStage->mByteCodeSize, NULL, &pShaderProgram->pDxDomainShader);
					ASSERT(SUCCEEDED(hr));
				} break;
				case SHADER_STAGE_GEOM: {
					pStage = &pDesc->mGeom;
					HRESULT hr = pRenderer->pDxDevice->CreateGeometryShader(pStage->pByteCode, pStage->mByteCodeSize, NULL, &pShaderProgram->pDxGeometryShader);
					ASSERT(SUCCEEDED(hr));
				} break;
				case SHADER_STAGE_FRAG: {
					pStage = &pDesc->mFrag;
					HRESULT hr = pRenderer->pDxDevice->CreatePixelShader(pStage->pByteCode, pStage->mByteCodeSize, NULL, &pShaderProgram->pDxPixelShader);
					ASSERT(SUCCEEDED(hr));
				} break;
				case SHADER_STAGE_COMP: {
					pStage = &pDesc->mComp;
					HRESULT hr = pRenderer->pDxDevice->CreateComputeShader(pStage->pByteCode, pStage->mByteCodeSize, NULL, &pShaderProgram->pDxComputeShader);
					ASSERT(SUCCEEDED(hr));
				} break;
				}

				d3d11_createShaderReflection(
					(uint8_t*)(pStage->pByteCode),
					(uint32_t)pStage->mByteCodeSize,
					stage_mask,
	...
	D3D11_CPU_ACCESS_FLAG util_determine_dx_cpu_access_flags(ResourceMemoryUsage mem)
	{
		...
		default:
			...
			return (D3D11_CPU_ACCESS_FLAG)(-1);
		}
	}
	...
	void addPipeline(Renderer* pRenderer, const GraphicsPipelineDesc* pDesc, Pipeline** ppPipeline)
	{
		...
		{
			pRenderer->pDxDevice->CreateInputLayout(input_elements, input_elementCount,
				pDesc->pShaderProgram->pDxInputSignature->GetBufferPointer(), pDesc->pShaderProgram->pDxInputSignature->GetBufferSize(),
				&pPipeline->pDxInputLayout);
		}
	...
	void set_shader_resources(ID3D11DeviceContext* pContext, ShaderStage used_stages, uint32_t reg, uint32_t count, ID3D11ShaderResourceView** pSRVs)
	{
		if (used_stages & SHADER_STAGE_VERT)
		...
		for (uint32_t i = 0; i < cmdCount; ++i)
		{
			...
			for (uint32_t cmdIndex = 0; cmdIndex < (uint32_t)cmds.size(); ++cmdIndex)
			{
				...
				case CMD_TYPE_cmdBindRenderTargets:
				{
					...
					pContext->OMSetRenderTargets(rt.renderTargetCount,
						rt.ppRenderTargets,
						rt.pDepthStencil);
					for (uint32_t i = 0; i < rt.renderTargetCount; ++i)
					{
						...
						{
							FLOAT clear[4] = {
								rt.mLoadActions.mClearColorValues[i].r,
								rt.mLoadActions.mClearColorValues[i].g,
								rt.mLoadActions.mClearColorValues[i].b,
								rt.mLoadActions.mClearColorValues[i].a
							};
							pContext->ClearRenderTargetView(rt.ppRenderTargets[i], clear);
						}
					}
					if (rt.pDepthStencil && (rt.mLoadActions.mLoadActionDepth == LOAD_ACTION_CLEAR ||
						rt.mLoadActions.mLoadActionStencil == LOAD_ACTION_CLEAR))
					{
						...
						pContext->ClearDepthStencilView(rt.pDepthStencil, clearFlag, rt.mLoadActions.mClearDepth.depth, rt.mLoadActions.mClearDepth.stencil);
					}
					break;
				}
				...
				case CMD_TYPE_cmdBindDescriptors:
				{
					...
					for (uint32_t i = 0; i < pRootSignature->mStaticSamplerCount; ++i)
					{
						set_samplers(pContext, pRootSignature->pStaticSamplerStages[i], pRootSignature->pStaticSamplerSlots[i], 1, &pRootSignature->ppStaticSamplers[i]);
					}
					/************************************************************************/
					...
					for (uint32_t i = 0; i < bind.numDescriptors; ++i)
					{
						...
						{
							ID3D11ShaderResourceView** pSRVs = (ID3D11ShaderResourceView**)alloca(pParam->mCount * sizeof(ID3D11ShaderResourceView*));
						...
						{
							ID3D11UnorderedAccessView** pUAVs = (ID3D11UnorderedAccessView**)alloca(pParam->mCount * sizeof(ID3D11UnorderedAccessView*));
							for (uint32_t i = 0; i < pParam->mCount; ++i)
						...
						case DESCRIPTOR_TYPE_UNIFORM_BUFFER:
						{
							if (pParam->pOffsets || pParam->pSizes)
							{
								...
								for (uint32_t i = 0; i < pParam->mCount; ++i)
								{
									...
									set_constant_buffers(pContext, pRes->used_stages, pRes->reg + i, 1, &pCmd->pTransientConstantBuffer->pDxResource);
								}
							}
							else
							{
				...
				{
					pContext->IASetIndexBuffer((ID3D11Buffer*)cmd.mBindIndexBufferCmd.pBuffer->pDxResource,
						cmd.mBindIndexBufferCmd.pBuffer->mDxIndexFormat,
						cmd.mBindIndexBufferCmd.offset);
					break;
				}
				case CMD_TYPE_cmdBindVertexBuffer:
				{
					pContext->IASetVertexBuffers(0, cmd.mBindVertexBufferCmd.bufferCount,
						cmd.mBindVertexBufferCmd.ppBuffers,
						cmd.mBindVertexBufferCmd.pStrides,
				...
				{
					pContext->DrawInstanced(cmd.mDrawInstancedCmd.vertexCount,
						cmd.mDrawInstancedCmd.instanceCount,
						cmd.mDrawInstancedCmd.firstVertex, cmd.mDrawInstancedCmd.firstInstance);
					break;
				}
				...
				{
					pContext->DrawIndexedInstanced(cmd.mDrawIndexedInstancedCmd.indexCount,
						cmd.mDrawIndexedInstancedCmd.instanceCount,
						cmd.mDrawIndexedInstancedCmd.firstIndex,
						cmd.mDrawIndexedInstancedCmd.firstVertex,
				...
				case CMD_TYPE_cmdResourceBarrier:
					break;
				case CMD_TYPE_cmdSynchronizeResources:
					break;
				case CMD_TYPE_cmdFlushBarriers:
					break;
				case CMD_TYPE_cmdExecuteIndirect:
					break;
				...
				case CMD_TYPE_cmdUpdateBuffer:
				{
					...
					pContext->UpdateSubresource(update.pBuffer->pDxResource, 0, NULL,
						((uint8_t*)sub.pData) + update.srcOffset, sub.RowPitch, sub.DepthPitch);
				...
				case CMD_TYPE_cmdUpdateSubresources:
				{
					...
					for (uint32_t i = 0; i < update.numSubresources; ++i)
					{
						...
						pContext->UpdateSubresource(update.pTexture->pDxResource, i, NULL,
							((uint8_t*)pData->pData), pData->mRowPitch, pData->mSlicePitch);
