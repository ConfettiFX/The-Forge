...
typedef uint32_t (*PFN_HOOK_ADD_DESCRIPTIOR_HEAP)(
	D3D12_DESCRIPTOR_HEAP_TYPE type,
	uint32_t numDescriptors); 

typedef void(*PFN_HOOK_POST_INIT_RENDERER)(
	Renderer* pRenderer);

typedef void(*PFN_HOOK_ADD_BUFFER)(
	Buffer* pBuffer,
	D3D12_RESOURCE_DESC& desc);

typedef void(*PFN_HOOK_ENABLE_DEBUG_LAYER)(
	Renderer* pRenderer);

typedef void(*PFN_HOOK_HEAP_DESC)(
	uint32_t resourceAllocMemReqFlags,
...
typedef uint32_t(*PFN_HOOK_GET_SWAP_CHAIN_IMAGE_INDEX)(
	SwapChain* pSwapChain);

typedef void(*PFN_HOOK_SHADER_COMPILE_FLAGS)(
