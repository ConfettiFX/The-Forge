...
void AnimatedObject::Initialize(Rig* rig, Animation* animation)
{
	...
	totalSize += (sizeof(Matrix4) + sizeof(Vector3)) * rig->mNumJoints;
#endif
...
#ifdef ENABLE_FORGE_ANIMATION_DEBUG
	mBoneWorldMats = ozz::span<Matrix4>((Matrix4*)(mLocalTrans.data() + rig->mNumSoaJoints), rig->mNumJoints); //-V1027
	mJointScales = ozz::span<Vector3>((Vector3*)(mBoneWorldMats.data() + rig->mNumJoints), rig->mNumJoints); //-V1027
#endif
...
#ifdef ENABLE_FORGE_ANIMATION_DEBUG
		mBoneWorldMats[i] = Matrix4::identity();
