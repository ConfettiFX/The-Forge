...
inline bool loadBASISTextureDesc(FileStream* pStream, TextureDesc* pOutDesc, void** ppOutData, uint32_t* pOutDataSize)
{
    ...
    if (isPowerOf2(textureDesc.mWidth) && isPowerOf2(textureDesc.mHeight))
    {
        ...
        basisTextureFormat = imageinfo.m_alpha_flag ? basist::transcoder_texture_format::cTFPVRTC1_4_RGB
    ...
    uint32_t requiredSize = util_get_surface_size(textureDesc.mFormat, textureDesc.mWidth, textureDesc.mHeight, textureDesc.mDepth, 1, 1, 0,
                                                  textureDesc.mMipLevels, 0, textureDesc.mArraySize);
    ...
    for (uint32_t s = 0; s < fileinfo.m_total_images; ++s)
    {
        ...
        for (uint32_t m = 0; m < fileinfo.m_image_mipmap_levels[s]; ++m)
        {
            ...
            if (!decoder.transcode_image_level(basisData, (uint32_t)memSize, s, m, data,
                                               (uint32_t)(rowPitchInBlocks * imageinfo.m_num_blocks_y), basisTextureFormat, 0,
                                               rowPitchInBlocks))
            {
