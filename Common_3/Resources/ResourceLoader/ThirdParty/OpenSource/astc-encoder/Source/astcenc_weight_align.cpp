...
void prepare_angular_tables()
{
	for (unsigned int i = 0; i < ANGULAR_STEPS; i++)
	{
		...
		{
			sin_table[j][i] = static_cast<float>(sinf((2.0f * astc::PI_ / (SINCOS_STEPS - 1.0f)) * angle_step * static_cast<float>(j)));
			cos_table[j][i] = static_cast<float>(cosf((2.0f * astc::PI_ / (SINCOS_STEPS - 1.0f)) * angle_step * static_cast<float>(j)));
		}
	}
}
...
static void compute_angular_offsets(
	...
	vfloat mult = vfloat(1.0f / (2.0f * astc::PI_));
