...
static const ImS8   IM_S8_MIN  = INT8_MIN;
static const ImS8   IM_S8_MAX  = INT8_MAX;
static const ImU8   IM_U8_MIN  = 0;
static const ImU8   IM_U8_MAX  = UINT8_MAX;
static const ImS16  IM_S16_MIN = INT16_MIN;
static const ImS16  IM_S16_MAX = INT16_MAX;
static const ImU16  IM_U16_MIN = 0;
static const ImU16  IM_U16_MAX = UINT16_MAX;
static const ImS32  IM_S32_MIN = INT32_MIN;  // (-2147483647 - 1), (0x80000000);
static const ImS32  IM_S32_MAX = INT32_MAX;  // (2147483647), (0x7FFFFFFF)
static const ImU32  IM_U32_MIN = 0;
static const ImU32  IM_U32_MAX = UINT32_MAX; // (0xFFFFFFFF)
static const ImS64  IM_S64_MIN = INT64_MIN;  // (-9223372036854775807ll - 1ll);
static const ImS64  IM_S64_MAX = INT64_MAX;  // (9223372036854775807ll);
static const ImU64  IM_U64_MIN = 0;
static const ImU64  IM_U64_MAX = UINT64_MAX; // (0xFFFFFFFFFFFFFFFFull);
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        value_changed |= DragScalar("", data_type, p_data, v_speed, p_min, p_max, perComponentFormat[i], flags);
        PopID();
        PopItemWidth();
...
bool ImGui::SliderScalarN(const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= SliderScalar("", data_type, v, v_min, v_max, perComponentFormat[i], flags);
        PopID();
        PopItemWidth();
...
bool ImGui::InputScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= InputScalar("", data_type, p_data, p_step, p_step_fast, perComponentFormat[i], flags);
        PopID();
        PopItemWidth();
        p_data = (void*)((char*)p_data + type_size);
    }
...
#include "../../../../Utilities/ThirdParty/OpenSource/Nothings/stb_textedit.h"
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_cut || is_copy)
        {
            ...
            if (io.SetClipboardTextFn)
            {
                ...
                char* clipboard_data = (char*)tf_malloc(clipboard_data_len * sizeof(char));
        ...
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                ImWchar* clipboard_filtered = (ImWchar*)tf_malloc((clipboard_len + 1) * sizeof(ImWchar));
...
static void ColorEditRestoreHS(const float* col, float* H, float* S, float* V)
{
    ...
    ImGuiContext& g = *GImGui;
    if (g.ColorEditLastColor != ImGui::ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 0.0f)))
        return;
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (value_changed && picker_active_window == NULL)
    {
        ...
        if ((flags & ImGuiColorEditFlags_DisplayHSV) && (flags & ImGuiColorEditFlags_InputRGB))
        {
            ...
            ColorConvertHSVtoRGB(f[0], f[1], f[2], f[0], f[1], f[2]);
            g.ColorEditLastColor = ColorConvertFloat4ToU32(ImVec4(f[0], f[1], f[2], 0.0f));
        }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        if (IsItemActive())
        {
            ...
            if (g.ColorEditLastColor == ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 0.0f)))
                H = g.ColorEditLastHue;
            value_changed = value_changed_sv = true;
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    if (value_changed_h || value_changed_sv)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ...
            g.ColorEditLastColor = ColorConvertFloat4ToU32(ImVec4(col[0], col[1], col[2], 0.0f));
        }
        else if (flags & ImGuiColorEditFlags_InputHSV)
        {
            col[0] = H;
    ...
    ImVec4 hue_color_f(1.0f, 1.0f, 1.0f, style.Alpha); ColorConvertHSVtoRGB(H, 1, 1, hue_color_f.x, hue_color_f.y, hue_color_f.z);
