...
void ImGui::TableBeginInitMemory(ImGuiTable* table, int columns_count)
{
    ...
    table->RawData = tf_malloc(span_allocator.GetArenaSizeInBytes());
...
const char* ImGui::TableGetColumnName(const ImGuiTable* table, int column_n)
{
    ...
    if (column->NameOffset == -1)
        return "";
    return table->ColumnsNames.begin() + column->NameOffset;
}
...
{
    tf_placement_new<ImGuiTableSettings>(settings);
    ImGuiTableColumnSettings* settings_column = settings->GetColumnSettings();
    for (int n = 0; n < columns_count_max; n++, settings_column++)
        tf_placement_new<ImGuiTableColumnSettings>(settings_column);
    settings->ID = id;
