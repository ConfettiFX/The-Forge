...
#define IMGUI_DEBUG_LOG(...)            LOGF(eINFO, __VA_ARGS__)
...
// Helpers: Hashing
inline ImGuiID       ImHashData(const void* data, size_t data_size, ImU32 seed = 0) { return (uint32_t)tf_mem_hash<uint8_t>((const uint8_t*)data, data_size, seed); }
inline ImGuiID       ImHashStr(const char* data, size_t data_size = 0, ImU32 seed = 0) { return (uint32_t)tf_mem_hash<uint8_t>((const uint8_t*)data, data_size == 0 ? strlen(data) : data_size, seed); }
...
#endif
IMGUI_API void*             ImFileLoadToMemory(const char* filename, FileMode mode, size_t* out_file_size = NULL, int padding_bytes = 0);
...
static inline ImVec2 ImClamp(const ImVec2& v, const ImVec2& mn, ImVec2 mx)      { return clamp(v, mn, mx); }
...
static inline float  ImFloor(float f)                                           { return (float)(ImS32)(f); }
static inline float  ImFloorSigned(float f)                                     { return (float)((f >= 0 || (float)(ImS32)f == f) ? (ImS32)f : (ImS32)f - 1); } // Decent replacement for floorf()
static inline ImVec2 ImFloor(const ImVec2& v)                                   { return ImVec2((float)(ImS32)(v.x), (float)(ImS32)(v.y)); }
...
struct ImPool
{
    ...
    T* Add() { int idx = FreeIdx; if (idx == Buf.Size) { Buf.resize(Buf.Size + 1); FreeIdx++; } else { FreeIdx = *(int*)&Buf[idx]; } tf_placement_new<T>(&Buf[idx]); AliveCount++; return &Buf[idx]; }
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    ~ImGuiViewportP()                   { if (DrawLists[0]) tf_delete(DrawLists[0]); if (DrawLists[1]) tf_delete(DrawLists[1]); }
...
struct ImGuiContext
{
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas)
    {
        ...
        IO.Fonts = shared_font_atlas ? shared_font_atlas : tf_new(ImFontAtlas);
        Time = 0.0f;
        FrameCount = 0;
...
struct IMGUI_API ImGuiTabBar
{
    ...
    const char*         GetTabName(const ImGuiTabItem* tab) const
    {
        ...
        IM_ASSERT(tab->NameOffset != -1 && tab->NameOffset < (TabsNames.size() + 1));
        return TabsNames.begin() + tab->NameOffset;
    }
};
