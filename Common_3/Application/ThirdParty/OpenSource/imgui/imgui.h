...
#ifndef ImDrawIdx
typedef uint16_t ImDrawIdx;   // Default: 16-bit (for maximum compatibility with renderer backends)
#endif
...
typedef uint16_t ImWchar16;   // A single decoded U16 character/code point. We encode them as multi bytes UTF-8 when used in strings.
typedef uint32_t ImWchar32;     // A single decoded U32 character/code point. We encode them as multi bytes UTF-8 when used in strings.
...
namespace ImGui
{
    ...
    IMGUI_API void          DestroyDemoWindow();                        // destroy Demo window resources
    ...
    IMGUI_API void          ProgressBar(float fraction, const ImVec2& size_arg = ImVec2(-FLT_MIN, 0.0f), const char* overlay = NULL);
...
struct ImVector
{
    ...
    inline ~ImVector()                                      { if (Data) tf_free(Data); Data = NULL; } // Important: does not destruct anything

    inline void         clear()                             { if (Data) { Size = Capacity = 0; tf_free(Data); Data = NULL; } }  // Important: does not destruct anything
    inline void         clear_delete()                      { for (int n = 0; n < Size; n++) tf_delete(Data[n]); clear(); }     // Important: never called automatically! always explicit.
    ...
    inline void         reserve(int new_capacity)           { if (new_capacity <= Capacity) return; T* new_data = (T*)tf_malloc((size_t)new_capacity * sizeof(T)); if (Data) { memcpy(new_data, Data, (size_t)Size * sizeof(T)); tf_free(Data); } Data = new_data; Capacity = new_capacity; }
    inline void         reserve_discard(int new_capacity)   { if (new_capacity <= Capacity) return; if (Data) tf_free(Data); Data = (T*)tf_malloc((size_t)new_capacity * sizeof(T)); Capacity = new_capacity; }
...
struct ImGuiTextBuffer
{
    ...
    ImGuiTextBuffer()                           { Buf = bempty(); }
    ~ImGuiTextBuffer()                          { if (bisvalid(&Buf)) bdestroy(&Buf); }
    inline char         operator[](int i) const { IM_ASSERT(bisvalid(&Buf)); return bchar(&Buf, i); }
    const char*         begin() const           { return bisvalid(&Buf) ? bdata(&Buf) : EmptyString; }
    const char*         end() const             { return bisvalid(&Buf) ? bdataofs(&Buf, blength(&Buf)) : EmptyString; }   // Buf is zero-terminated, so end() will point on the zero-terminator
    int                 size() const            { return blength(&Buf); }
    int                 capacity() const        { return bcapacity(&Buf); }
    bool                empty() const           { return blength(&Buf) == 0; }
    void                clear()                 { if (bisvalid(&Buf)) btrunc(&Buf, 0); }
    void                reserve(int capacity)   { balloc(&Buf, capacity); }
    const char*         c_str() const           { return begin(); }
    IMGUI_API void      append(const char* str);
