...
public:
	 ...
	 * \return true if the button may be processed by listeners with a lower priority, false otherwise.
	 */
	virtual bool OnDeviceButtonFloat(float deltaTime, DeviceId device, DeviceButtonId deviceButton, float oldValue, float newValue) { GAINPUT_UNUSED(deltaTime); GAINPUT_UNUSED(device); GAINPUT_UNUSED(deviceButton); GAINPUT_UNUSED(oldValue); GAINPUT_UNUSED(newValue); return true; }
