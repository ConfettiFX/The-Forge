...
#include "../../../include/gainput/apple/GainputMac.h"

#include "../../../source/gainput/apple/GainputInputDeviceMouseMac.h"
#include "../../../source/gainput/apple/GainputInputDeviceMouseMacRaw.h"
#include "../../../source/gainput/apple/GainputInputDeviceKeyboardMac.h"
...
- (void) reportMouseMove:(NSEvent*) nsEvent
{
	...
	NSView * forgeView = (__bridge NSView*)parentView;
	mRetinaScale = [forgeView.window backingScaleFactor];
...
- (void) flagsChanged: (NSEvent*) nsEvent
{
	...
	// check for keycode == 0
	// Weird edge case when taking screenshots
	// Seems to be common when key combinations trigger applescripts.
	// Skip if that's the case. We still receive the correct key events
	// regardless of this one.
	if(!keyboard || nsEvent.keyCode == 0)
		return;
