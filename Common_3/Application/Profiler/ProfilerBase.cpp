...
static const char* pReferenceTimesNames[] = { "1ms", "2ms", "5ms", "10ms", "15ms", "20ms", "33ms", "66ms", "100ms", "250ms", "500ms", "1000ms" };
...
void profileDrawPlotMode(Profile& S)
{
	...
	for (ptrdiff_t i = 0; i < arrlen(gPlotModeData); ++i)
	{
		...
		UIWidget* pOneLineCheckbox = uiCreateComponentWidget(pWidgetUIComponent, timerInfo.pName, &oneLineCheckbox, WIDGET_TYPE_ONE_LINE_CHECKBOX);
...
{
#ifdef ENABLE_FORGE_UI
	const char* headerNames[gColumnsCount] =
	{ 
		"Group/Timer",  "Time",       "Average Time",   "Max Time",          "Min Time",
	...
	luaRegisterWidget(uiCreateComponentWidget(pWidgetUIComponent, "Header", &headerRow, WIDGET_TYPE_COLUMN));
	...
	for (uint32_t groupIndex = 0; groupIndex < S.nGroupCount; ++groupIndex)
	{
		...
		for (uint32_t timerIndex = 0; timerIndex < S.nTotalTimers; ++timerIndex)
		{
			if (S.TimerInfo[timerIndex].nGroupIndex == groupIndex)
			{
				...
				ASSERT(strlen(S.TimerInfo[timerIndex].pName) < sizeof(labelBase.mLabel));
...
void profileUpdateTimerModeData(Profile& S, uint32_t groupIndex, uint32_t timerIndex, uint32_t tableLocation)
{
	...
	{
		fTime, fAverage, fMax, fMin, fCallAverage, (float)fCallCount,
		fFrameMsExclusive, fAverageExclusive, fMaxExclusive
	};
...
float2 cmdDrawCpuProfile(Cmd* pCmd, float2 screenCoordsInPx, FontDrawDesc* pDrawDesc)
{
	...
	//print cpu time
	screenCoordsInPx.y += fntMeasureFontText(gGpuProfileTitleText, pDrawDesc).y + gDefaultGpuProfileDrawDesc.mHeightOffset;
...
void profileLoadWidgetUI(Profile& S)
{
	...
	luaRegisterWidget(uiCreateComponentWidget(pWidgetUIComponent, "topmenu", &topMenuRow, WIDGET_TYPE_COLUMN));
...
void ProfileDumpHtml(ProfileWriteCallback CB, void* Handle, int nMaxFrames, const char* pHost)
{
	...
	for (uint32_t i = 0; i < S.nTotalTimers; ++i)
	{
		...
		ProfilePrintf(
			CB, Handle, "%8d:%s(%s)\n", nTimerCounter[idx], S.TimerInfo[idx].pName, S.GroupInfo[S.TimerInfo[idx].nGroupIndex].pName); //-V595
	}
	ProfilePrintf(CB, Handle, "\n-->\n");
...
void dumpBenchmarkData(IApp::Settings* pSettings, const char* outFilename, const char * appName)
{
	...
	bformat(&name, "%s_%s", appName, outFilename);
	...
	if (fsOpenStreamFromPath(RD_LOG, (const char*)name.data, FM_WRITE, NULL, &statsFile))
	{
		...
		bformata(&output, "\"VendorID\": \"%s\", \n", g_Profile.pGpuSettings->mGpuVendorPreset.mVendorId);
		bformata(&output, "\"ModelID\": \"%s\", \n\n", g_Profile.pGpuSettings->mGpuVendorPreset.mModelId);
