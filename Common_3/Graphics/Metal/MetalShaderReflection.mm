...
struct BufferInfo
{
	...
	BufferInfo(): bufferIndex(-1), sizeInBytes(0), alignment(0), currentOffset(0), isUAV(false), isArgBuffer(false) {}
};
...
static uint32_t reflectShaderStruct(ShaderReflectionInfo* info, unsigned int bufferIndex, unsigned long parentOffset, MTLStructType* structObj)
{
...
static void reflectShaderBufferArgument(ShaderReflectionInfo* info, MTLArgument* arg)
{
...
static void reflectShaderSamplerArgument(ShaderReflectionInfo* info, MTLArgument* arg)
{
...
static void reflectShaderTextureArgument(ShaderReflectionInfo* info, MTLArgument* arg)
{
...
static void reflectShader(ShaderReflectionInfo* info, NSArray<MTLArgument*>* shaderArgs)
{
...
void mtl_createShaderReflection(
	Renderer* pRenderer, Shader* shader, const uint8_t* shaderCode, uint32_t shaderSize, ShaderStage shaderStage, ShaderReflection* pOutReflection)
{
		...
		for (ptrdiff_t i = 0; i < arrlen(reflectionInfo.buffers); ++i)
		{
			...
			if (!isInputVertexBuffer(bufferInfo, shaderStage))
			{
				if (bufferInfo.isArgBuffer)
				{
					...
					addShaderResource(
						pResources, resourceIdx, DESCRIPTOR_TYPE_UNDEFINED, bufferInfo.bufferIndex, bufferInfo.sizeInBytes,
