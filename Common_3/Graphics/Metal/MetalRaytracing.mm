...
//////////////////////////////////////////////////////////////////////////////////
struct ASOffset{
	unsigned* vbGeometriesOffsets;
	unsigned* ibGeometriesOffsets;
	unsigned vbSize;
	unsigned ibSize;
								 ...
								 ASOffset* pOutOffset)
{
	...
	{
		arrsetlen(pOutOffset->vbGeometriesOffsets, pDesc->mBottomASDesc->mDescCount);
		arrsetlen(pOutOffset->ibGeometriesOffsets, pDesc->mBottomASDesc->mDescCount);
		pOutOffset->vbSize = 0;
		pOutOffset->ibSize = 0;
		pOutOffset->trianglesCount = 0;
		pOutOffset->vertexStride = sizeof(float3);
		for (unsigned j = 0; j < pDesc->mBottomASDesc->mDescCount; ++j)
		{
			pOutOffset->vbGeometriesOffsets[j] = vbOffset;
			pOutOffset->ibGeometriesOffsets[j] = ibOffset;
			...
			pOutOffset->vbSize += pDesc->mBottomASDesc->pGeometryDescs[j].mVertexCount * sizeof(float3);
			...
			if (ibDstPtr != NULL)
			{
				...
				pOutOffset->ibSize += pDesc->mBottomASDesc->pGeometryDescs[j].mIndexCount * indexSize;
				pOutOffset->trianglesCount += pDesc->mBottomASDesc->pGeometryDescs[j].mIndexCount / 3;
			} else
			{
				pOutOffset->trianglesCount += pDesc->mBottomASDesc->pGeometryDescs[j].mVertexCount / 3;
			}
...
{
	uint32_t* instancesIndices = NULL;
	arrsetlen(instancesIndices, pDesc->mInstancesDescCount);
	for (unsigned i = 0; i < pDesc->mInstancesDescCount; ++i)
	...
	memcpy(instanceBuffer.contents, instancesIndices, instanceBufferLength);
	arrfree(instancesIndices);
...
void mtl_addAccelerationStructure(Raytracing* pRaytracing, const AccelerationStructureDescTop* pDesc, AccelerationStructure** ppAccelerationStructure)
{
	...
	for (unsigned i = 0; i < pDesc->mInstancesDescCount; ++i)
	{
		...
		bool found = false;
		for (ptrdiff_t i = 0; i < arrlen(AS->mActiveHitGroups); ++i)
		{
			if (AS->mActiveHitGroups[i] == hitID)
			{
				found = true;
				break;
			}
		...
		if (!found)
			arrpush(AS->mActiveHitGroups, hitID);
	}
	{
		...
		arrfree(offset.vbGeometriesOffsets);
		arrfree(offset.ibGeometriesOffsets);
	}
...
void mtl_removeAccelerationStructure(Raytracing* pRaytracing, AccelerationStructure* pAccelerationStructure)
{
	...
	arrfree(pAccelerationStructure->mActiveHitGroups);
...
typedef struct DispatchedShaderSet
{
	uint16_t key;
}DispatchedShaderSet;

API_AVAILABLE(macos(10.14), ios(12.0))
static void dispatchShader(id<MTLComputeCommandEncoder> computeEncoder, RaytracingPipeline* pPipeline, uint16_t shaderIndex, DispatchedShaderSet** pDispatchedShaders)
{
	DispatchedShaderSet* dispatchedShaders = *pDispatchedShaders;
	if (hmgetp_null(dispatchedShaders, shaderIndex) != NULL)
	...
	DispatchedShaderSet val = {shaderIndex};
	hmputs(dispatchedShaders, val);
	*pDispatchedShaders = dispatchedShaders;
}
				   ...
				   MTLSize threadgroups)
{
	...
	DispatchedShaderSet* dispatchedShaders = NULL;
