...
// CONFFX: We do not support microprofile when dealing with multiple runtime graphic apis
// CONFFX: We do not support xbox and mac yet
#if defined(_DURANGO) || defined(METAL) || defined(ENABLE_RENDERER_RUNTIME_SWITCH)
#undef ENABLE_MICRO_PROFILER
#define ENABLE_MICRO_PROFILER 0
#else
#undef ENABLE_MICRO_PROFILER
...
// CONFFX_BEGIN
#define MICROPROFILE_GPU_ENTER_TOKEN(token) MicroProfileEnterGpu(token)
#define MICROPROFILE_GPU_LEAVE() MicroProfileLeaveGpu()
// CONFFX_END
...
MICROPROFILE_API const char* MicroProfileGetLabel(uint32_t eType, uint64_t nLabel);
...
MICROPROFILE_API void MicroProfileGpuInitInternal(struct ID3D11Device* pDevice);
MICROPROFILE_API void MicroProfileGpuInitInternal(struct ID3D12Device* pDevice, struct ID3D12CommandQueue* pCommandQueue);
MICROPROFILE_API void MicroProfileGpuInitInternal(struct VkDevice_T* pDevice, struct VkPhysicalDevice_T* pPhysicalDevice, struct VkQueue_T* pQueue);
...
MICROPROFILE_API void MicroProfileEnterGpu(MicroProfileToken nToken);
...
void MicroProfileInit()
{
	...
	if(bOnce)
	{
		...
		strcpy_s(&S.CategoryInfo[0].pName[0], MICROPROFILE_NAME_MAX_LEN, "default");
		S.nCategoryCount = 1;
...
{
	MicroProfileThreadLog* pLog = MicroProfileGetOrCreateThreadLog();
	MP_ASSERT(pLog != nullptr); // check not null
	...
	pScopeState->nTick = MicroProfileEnter(nToken);
}

void MicroProfileLeaveGpu()
{
	MicroProfileThreadLog* pLog = MicroProfileGetOrCreateThreadLog();
	MP_ASSERT(pLog != nullptr); // check not null
	MP_ASSERT(pLog->nStackScope > 0); // if youre hitting this assert you probably have mismatched _ENTER/_LEAVE markers
	MicroProfileScopeStateC* pScopeState = &pLog->ScopeState[--pLog->nStackScope];
	MicroProfileLeave(pScopeState->Token, pScopeState->nTick);
}
				...
				for(uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; ++i)
				{
					...
					if(pLog->nGpu)
					{
						...
						for(uint32_t j = 0; j < 2; ++j)
						{
							...
							for(uint32_t k = nStart; k < nEnd; ++k)
							{
								...
								uint64_t Type = MicroProfileLogType(L);

								if(Type == MP_LOG_ENTER || Type == MP_LOG_LEAVE)
								{
									uint32_t nTimer = (uint32_t)MicroProfileLogGetTick(L);
					...
					for(uint32_t j = 0; j < 2; ++j)
					{
						...
						for(uint32_t k = nStart; k < nEnd; ++k)
						{
							...
							{
								uint64_t nTimer = MicroProfileLogTimerIndex(LE);
							...
							else if(MP_LOG_LEAVE == nType)
							{
								...
								{									
									...
									uint32_t nTimerIndex = (uint32_t)MicroProfileLogTimerIndex(LE);
...
void MicroProfileDumpHtml(MicroProfileWriteCallback CB, void* Handle, int nMaxFrames, const char* pHost)
{
	...
	{
		nTickStartGpu = (nTickStart - nTickReferenceCpu) * int64_t(double(nTicksPerSecondGpu) / double(nTicksPerSecondCpu)) + nTickReferenceGpu;
	}
	...
	for(uint32_t i = 0; i < nNumFrames; ++i)
	{
		...
		for(uint32_t j = 0; j < MICROPROFILE_MAX_THREADS; ++j)
		{
			...
			{
				uint32_t nLogType = (uint32_t)MicroProfileLogType(pLog->Log[k]);
				if(nLogType == MP_LOG_META)
				{
					// for meta, store the count + 8, which is the tick part
					nLogType = uint32_t(8 + MicroProfileLogGetTick(pLog->Log[k]));
				}
...
void* MicroProfileTraceThread(void* unused)
{
	...
	{
		FILE* pFile;
		fopen_s(&pFile, "\\\\.\\pipe\\microprofile-contextswitch", "rb");
		if(!pFile)
		{
			Sleep(1000);
			continue;
...
#endif
#endif
#endif// #if ENABLE_MICRO_PROFILER 	
