...
void getIncludeFiles(char newFilePath[], int *pIncludedCounter, const char* pIncluded[MAX_INCLUDE_FILE], std::string originFile)
{	
	char toCstr[65536];
	...
	while (true)
	{
		...
		char newTempFilePath[256];
		strcpy(newTempFilePath, newFilePath);
		strcat(newTempFilePath, includeFile);

		std::string newContents = ReadFile(newTempFilePath);

		//check first it also has Include files
		getIncludeFiles(newFilePath, pIncludedCounter, pIncluded, newContents);
...
int main( int argc, char* argv[] )
{
	...
	getIncludeFiles(newFilePath, &includedCounter, pIncluded, source);

	char RESULT[65536];
	parser.ParserEntry(RESULT, fileName, source.data(), source.size(), entryName, shader, _language, pIncluded, includedCounter);
