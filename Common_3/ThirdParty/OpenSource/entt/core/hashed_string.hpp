...
class HashedString final {
    ...
    static constexpr uint64 helper(uint64 partial, const char *str) ENTT_NOEXCEPT {
        return str[0] == 0 ? partial : helper((partial^str[0])*prime, str+1);
    }

public:
    /*! @brief Unsigned integer type. */
    using hash_type = uint64;
 ...
 */
constexpr entt::HashedString operator"" ENTT_HS_SUFFIX(const char *str, size_t) ENTT_NOEXCEPT {
