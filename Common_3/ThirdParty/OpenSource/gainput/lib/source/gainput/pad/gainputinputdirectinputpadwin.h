...
#define guide "guide"
	...
	*/
	...
	public:
		...
		void DInputClean(int index)
		{
			if (gamePads.directInputCountConnected)
			{
				gamePadsInUse--;
				gamePads.directInputCountConnected--;
				gamePads.gamePadInfos[index].created = false;
				gamePads.gamePadInfos[index].gamepadDevicePtr = NULL;
				gamePads.gamePadInfos[index].interfacePtr = NULL;
				gamePads.gamePadInfos[index].gamepad.Clean();
				gamePads.gamePadInfos[index].assignedHid = false;
				gamePads.gamePadInfos[index].rawInputDeviceHandle = NULL;
				if (gamePads.gamePadInfos[index].hidDevice)
				{
					CloseHandle(gamePads.gamePadInfos[index].hidDevice);
					gamePads.gamePadInfos[index].hidDevice = NULL;
				}
			}
		}
	...
	public:
		...
		{
			if (DirectInputInitializer::GetInstance(handle)->GetDirectInputGamePad(index, gamepadInfo))
			{
		...
		void OnDeviceRemove(int index, void* handle)
		{
			if (index == padIndex)
			{
				if (created)
				{
					...
					gamepadInfo.gamepadDevicePtr = NULL;
					gamepadInfo.assignedHid = false;
					memset(gamepad.name, 0, sizeof(gamepad.name));
					created = false;
					DirectInputInitializer::GetInstance()->DInputClean(gamepadInfo.padIndex);
				}
				else
				{
					//xinput disconnected??
					DirectInputInitializer::GetInstance()->XInputClean();
				}
			}
		}
		...
		{
			DirectInputInitializer::GetInstance()->OnDeviceAdd();
			if (DirectInputInitializer::GetInstance()->GetDirectInputGamePad(index, gamepadInfo))
			{
				gamepad = gamepadInfo.gamepad;
				created = true;
			}
