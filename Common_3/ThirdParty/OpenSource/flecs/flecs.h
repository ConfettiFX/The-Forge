...
#define ECS_COMPONENT_DEFINE(world, id) \
	do { \
		TF_INIT_ZERO(ecs_component_desc_t, cmp_desc); \
		cmp_desc.entity.entity = ecs_id(id); \
		cmp_desc.entity.name = #id; \
		cmp_desc.entity.symbol = #id; \
		cmp_desc.size = sizeof(id); \
		cmp_desc.alignment = ECS_ALIGNOF(id); \
		ecs_id(id) = ecs_component_init(world, &cmp_desc); \
...
#define ECS_TAG_DEFINE(world, id) \
	do { \
		ecs_entity_desc_t entt_desc = {}; \
		entt_desc.name = #id; \
		entt_desc.symbol = #id; \
		id = ecs_entity_init(world, &entt_desc); \
...
#define ECS_SYSTEM(world, id, kind, ...) \
	do { \
		...
		TF_INIT_ZERO(ecs_system_desc_t, sys_desc); \
		sys_desc.entity.name = #id; \
		sys_desc.entity.add[0] = kind; \
		sys_desc.query.filter.expr = #__VA_ARGS__; \
		sys_desc.callback = ecs_iter_action(id); \
		ecs_entity_t id = ecs_system_init(world, &sys_desc); \
