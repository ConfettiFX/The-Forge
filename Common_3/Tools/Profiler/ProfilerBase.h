...
//  }
//
//  Once code is instrumented the gui is activeted by calling ProfileToggleDisplayMode or by clicking in the upper left corner of
//  the screen
//
...
// 		uint32_t ProfileGpuInsertTimer(void* pContext);
// 		uint64_t ProfileGpuGetTimeStamp(uint32_t nKey);
// 		uint64_t ProfileTicksPerSecondGpu();
//  threading:
//      const char* ProfileGetThreadName(); Threadnames in detailed view
//
// Default implementations of Gpu timestamp functions:
// 		OpenGL:
// 			in .c file where PROFILE_IMPL is defined:
//   		#define PROFILE_GPU_TIMERS_GL 1
// 			call ProfileGpuInitGL() on startup
//		D3D11:
// 			in .c file where PROFILE_IMPL is defined:
//   		#define PROFILE_GPU_TIMERS_D3D11 1
// 			call ProfileGpuInitD3D11(Device) on startup
//		D3D12:
// 			in .c file where PROFILE_IMPL is defined:
//   		#define PROFILE_GPU_TIMERS_D3D12 1
// 			call ProfileGpuInitD3D12(Device, CommandQueue) on startup
//			single-threaded: call ProfileGpuSetContext(CommandList) every frame before issuing GPU markers
//			multi-threaded:
//				#define PROFILE_GPU_TIMERS_MULTITHREADED 1
//				on recording thread before using command list, call ProfileGpuBegin(CommandList)
//				on recording thread after you're done with command list, call work = ProfileGpuEnd()
//				when replaying, call ProfileGpuSubmit(work) in the same order as ExecuteCommandLists
//		Vulkan:
// 			in .c file where PROFILE_IMPL is defined:
//   		#define PROFILE_GPU_TIMERS_VK 1
// 			call ProfileGpuInitVK(Device, PhysicalDevice, Queue) on startup
...
#define PROFILE_SCOPE(var) ProfileScopeHandler PROFILE_TOKEN_PASTE(foo, __LINE__)(g_mp_##var)
#define PROFILE_SCOPE_TOKEN(token) ProfileScopeHandler PROFILE_TOKEN_PASTE(foo, __LINE__)(token)
#define PROFILE_SCOPEI(group, name, color) static ProfileToken PROFILE_TOKEN_PASTE(g_mp,__LINE__) = ProfileGetToken(group, name, color, ProfileTokenTypeCpu); ProfileScopeHandler PROFILE_TOKEN_PASTE(foo,__LINE__)( PROFILE_TOKEN_PASTE(g_mp,__LINE__))
#define PROFILE_SCOPEGPU(var) ProfileScopeHandler PROFILE_TOKEN_PASTE(foo, __LINE__)(g_mp_##var)
#define PROFILE_SCOPEGPUI(name, color) static ProfileToken PROFILE_TOKEN_PASTE(g_mp,__LINE__) = ProfileGetToken("GPU", name, color,  ProfileTokenTypeGpu); ProfileScopeHandler PROFILE_TOKEN_PASTE(foo,__LINE__)( PROFILE_TOKEN_PASTE(g_mp,__LINE__))
...
// Starts GPU profiling, inserts a value into the stack
#define PROFILE_GPU_ENTER(var, name, color) PROFILE_DEFINE_GPU(var, name, color); ProfileEnterGpu(g_mp_##var)
// Ends GPU profiling, only last stack value
#define PROFILE_GPU_LEAVE() ProfileLeaveGpu()

#ifndef PROFILE_USE_THREAD_NAME_CALLBACK
#define PROFILE_USE_THREAD_NAME_CALLBACK 0
#endif
...
struct Profile;
struct ProfileScopeStateC;

//PROFILE_API void ProfileInit();
//PROFILE_API void ProfileShutdown();
...
PROFILE_API void ProfileGpuInitInternal(struct ID3D11Device* pDevice);
PROFILE_API void ProfileGpuInitInternal(struct ID3D12Device* pDevice, struct ID3D12CommandQueue* pCommandQueue);
PROFILE_API void ProfileGpuInitInternal(struct VkDevice_T* pDevice, struct VkPhysicalDevice_T* pPhysicalDevice, struct VkQueue_T* pQueue);
PROFILE_API void ProfileGpuShutdown();

PROFILE_API void ProfileGpuSetContext(void* pContext);
PROFILE_API void ProfileGpuBegin(void* pContext);
PROFILE_API uint64_t ProfileGpuEnd();
PROFILE_API void ProfileGpuSubmit(uint64_t nWork);

PROFILE_API uint32_t ProfileGpuInsertTimer(void* pContext);
...
PROFILE_API void ProfileEnterGpu(ProfileToken nToken);
PROFILE_API void ProfileLeaveGpu();


#if PROFILE_USE_THREAD_NAME_CALLBACK
PROFILE_API const char* ProfileGetThreadName();
#else
#define ProfileGetThreadName() "<Unknown>"
#endif
...
struct ProfileScopeHandler
{
	ProfileToken nToken;
	uint64_t nTick;
	ProfileScopeHandler(ProfileToken Token) :nToken(Token)
	{
...
#if defined(_WIN32) 
#define PROFILE_CONTEXT_SWITCH_TRACE 1
...
struct ProfileThreadLog
{
	...
	ProfileScopeStateC ScopeState[PROFILE_STACK_MAX];
	enum
	{
		THREAD_MAX_LEN = 64,
	};
...
#define PROFILE_GPU_STATE_IMPL(API) \
	...
	{ \
		...
		S.GPU.Flip = ProfileGpuFlip##API; \
		S.GPU.InsertTimer = ProfileGpuInsertTimer##API; \
