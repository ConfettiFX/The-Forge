...
#else
P_THREAD_LOCAL ProfileThreadLog* g_ProfileThreadLog = 0;
#endif

static bool g_bUseLock = false; /// This is used because windows does not support using mutexes under dll init(which is where global initialization is handled)

void ProfileShutdown();
struct ForceProfileShutdown
{
	~ForceProfileShutdown()
	{
		if (g_ProfileThreadLog)
		{
			if (g_ProfileThreadLog->Log)
				conf_free(g_ProfileThreadLog->Log);

			conf_free(g_ProfileThreadLog);
			g_ProfileThreadLog = nullptr;
		}
		...
		if (Thread::IsMainThread())
		{
			if (g_ProfileGpuLog)
			{
				if (g_ProfileGpuLog->Log)
					conf_free(g_ProfileGpuLog->Log);

				conf_free(g_ProfileGpuLog);
				g_ProfileGpuLog = nullptr;
			}
		}
...
P_THREAD_LOCAL static ForceProfileShutdown g_ForceProfileShutdown;

PROFILE_DEFINE(g_ProfileFlip, "Profile", "ProfileFlip", 0x3355ee);
PROFILE_DEFINE(g_ProfileThreadLoop, "Profile", "ThreadLoop", 0x3355ee);
PROFILE_DEFINE(g_ProfileClear, "Profile", "Clear", 0x3355ee);
PROFILE_DEFINE(g_ProfileAccumulate, "Profile", "Accumulate", 0x3355ee);
PROFILE_DEFINE(g_ProfileContextSwitchSearch, "Profile", "ContextSwitchSearch", 0xDD7300);
PROFILE_DEFINE(g_ProfileWebServerUpdate, "Profile", "WebServerUpdate", 0xDD7300);
...
void ProfileInit()
{
	...
	if (bOnce)
	{
		...
		ProfileThreadLog* pGpu = ProfileCreateThreadLog("GPU");
		g_ProfileGpuLog = pGpu;
		P_ASSERT(S.Pool[0] == pGpu);
		pGpu->nGpu = 1;
		pGpu->nThreadId = 0;
	}
	if (bUseLock)
...
void ProfileOnThreadExit()
{
	...
	if (pLog)
	{
		...
		P_ASSERT(nLogIndex < PROFILE_MAX_THREADS && nLogIndex > 0);
...
void ProfileEnterGpu(ProfileToken nToken)
{
	ProfileThreadLog* pLog = ProfileGetOrCreateThreadLog();
	P_ASSERT(pLog != nullptr); // check not null
	P_ASSERT(pLog->nStackScope < PROFILE_STACK_MAX); // if youre hitting this assert you probably have mismatched _ENTER/_LEAVE markers
	ProfileScopeStateC* pScopeState = &pLog->ScopeState[pLog->nStackScope++];
	pScopeState->Token = nToken;
	pScopeState->nTick = ProfileEnter(nToken);
}

void ProfileLeaveGpu()
{
	ProfileThreadLog* pLog = ProfileGetOrCreateThreadLog();
	P_ASSERT(pLog != nullptr); // check not null
	P_ASSERT(pLog->nStackScope > 0); // if youre hitting this assert you probably have mismatched _ENTER/_LEAVE markers
	ProfileScopeStateC* pScopeState = &pLog->ScopeState[--pLog->nStackScope];
	ProfileLeave(pScopeState->Token, pScopeState->nTick);
}
...
{
#if PROFILE_GPU_TIMERS_MULTITHREADED
	P_ASSERT(pLog != 0); //this assert is hit if ProfileOnCreateThread is not called
	uint32_t nPos = pLog->nPutGpu.load(std::memory_order_relaxed);
	if (nPos >= PROFILE_GPU_BUFFER_SIZE)
	{
		S.nOverflow = 100;
	}
	else
	{
		if (!pLog->LogGpu)
		{
			pLog->LogGpu = static_cast<ProfileLogEntry *>(conf_malloc(sizeof(ProfileLogEntry) * PROFILE_GPU_BUFFER_SIZE));
			memset(pLog->LogGpu, 0, sizeof(ProfileLogEntry) * PROFILE_GPU_BUFFER_SIZE);
			S.nMemUsage += sizeof(ProfileLogEntry) * PROFILE_GPU_BUFFER_SIZE;
		}
		pLog->LogGpu[nPos] = ProfileMakeLogIndex(nBegin, nToken_, nTick);
		pLog->nPutGpu.store(nPos + 1, std::memory_order_release);
	}
#else
	(void)pLog;

	ProfileLogPut(nToken_, nTick, nBegin, g_ProfileGpuLog);
#endif
}
...
uint64_t ProfileEnter(ProfileToken nToken_)
{
	...
	if (nGroupMask & S.nActiveGroup)
	{
		if (ProfileThreadLog* pLog = ProfileGetOrCreateThreadLog())
		{
			...
			{
				uint32_t nTimer = ProfileGpuInsertTimer(pLog->pContextGpu);
				if (nTimer != (uint32_t)-1)
				{
					ProfileLogPutGpu(nToken_, nTimer, P_LOG_ENTER, pLog);
...
void ProfileLeave(ProfileToken nToken_, uint64_t nTickStart)
{
	if (PROFILE_INVALID_TICK != nTickStart)
	{
		if (ProfileThreadLog* pLog = ProfileGetOrCreateThreadLog())
		{
			...
			{
				uint32_t nTimer = ProfileGpuInsertTimer(pLog->pContextGpu);
				ProfileLogPutGpu(nToken_, nTimer, P_LOG_LEAVE, pLog);
				ProfileLogPutGpu(pLog->nLogIndex, P_TICK(), P_LOG_GPU_EXTRA, pLog);
			}
			else
			{
...
void ProfileFlipCpu()
{
	...
	if (S.nRunning || S.nForceEnable)
	{
		...
		if (S.nRunning)
		{
			...
			{
				PROFILE_SCOPE(g_ProfileThreadLoop);
				for (uint32_t i = 0; i < PROFILE_MAX_THREADS; ++i)
				{
					ProfileThreadLog* pLog = S.Pool[i];
					if (!pLog)
						...
						for (uint32_t j = 0; j < 2; ++j)
						{
							...
							for (uint32_t k = nStart; k < nEnd; ++k)
							{
								...
								if (Type == P_LOG_ENTER || Type == P_LOG_LEAVE)
								{
									...
									uint64_t nTick = ProfileGpuGetTimeStamp(nTimer);
			...
			{
				PROFILE_SCOPE(g_ProfileAccumulate);
				for (uint32_t i = 0; i < S.nTotalTimers; ++i)
				{
...
void ProfileFlip()
{
	PROFILE_SCOPE(g_ProfileFlip);

	ProfileFlipGpu();
	ProfileFlipCpu();
}

void ProfileGpuSetContext(void* pContext)
{
	if (ProfileThreadLog* pLog = ProfileGetOrCreateThreadLog())
	{
...
void ProfileGpuBegin(void* pContext)
{
	if (ProfileThreadLog* pLog = ProfileGetOrCreateThreadLog())
	{
		P_ASSERT(!pLog->bActiveGpu);
...
uint64_t ProfileGpuEnd()
{
	if (ProfileThreadLog* pLog = ProfileGetThreadLog())
	{
		...
		P_ASSERT(nPutGpu >= nStartGpu);

		pLog->pContextGpu = 0;
		pLog->nStartGpu = nPutGpu;
		pLog->bActiveGpu = 0;

		P_ASSERT(PROFILE_GPU_BUFFER_SIZE <= 1 << 24);
...
void ProfileGpuSubmit(uint64_t nWork)
{
	if (!nWork)
		return;
	...
	P_ASSERT(nStart <= nEnd);

	ProfileThreadLog* pLog = S.Pool[nLogIndex];
	if (!pLog)
		return;

	P_ASSERT(nEnd <= pLog->nStartGpu);

	for (uint32_t i = nStart; i < nEnd; ++i)
	{
		ProfileLogEntry LE = pLog->LogGpu[i];

		ProfileLogPut(ProfileLogTimerIndex(LE), ProfileLogGetTick(LE), ProfileLogType(LE), g_ProfileGpuLog);
	}
}
...
void ProfileWebServerHandleRequest(MpSocket Connection)
{
	...
	PROFILE_SCOPE(g_ProfileWebServerUpdate);
...
{
	PROFILE_SCOPE(g_ProfileContextSwitchSearch);
	uint32_t nContextSwitchPut = S.nContextSwitchPut;
