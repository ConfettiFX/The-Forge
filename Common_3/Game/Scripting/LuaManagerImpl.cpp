...
LuaManagerImpl::LuaManagerImpl(lua_State* L):
	...
	m_Functions( nullptr ),
	m_UpdateFunctionNameBuf(),
	m_UpdatableScriptExitBuf(),
...
static int msghandler(lua_State* L)
{
	...
	ASSERT(!"Lua script error encountered during automated testing");
#endif
...
bool LuaManagerImpl::ReloadUpdatableScript()
{
	...
	return SetUpdatableScript(m_UpdatableScriptFile, m_UpdatableScriptFilePassword, 
		(const char*)m_UpdateFunctionName.data, (const char*)m_UpdatableScriptExitName.data);
}
...
void LuaStateWrap::GetStringArg(int argIdx, const char** result) 
{ 
	*result = lua_tostring(luaState, argIdx);
}
