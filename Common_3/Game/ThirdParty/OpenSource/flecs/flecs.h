...
// Not sure if there is a better way to do the TF_INIT_ZERO
#ifdef __cplusplus

#define TF_INIT_ZERO(T, var) T var = {}

#else

#if defined(NX64) || defined(ANDROID)
#define TF_INIT_ZERO(T, var) T var = {{ 0 }}
#else
#define TF_INIT_ZERO(T, var) T var = {0}
#endif
...
#pragma GCC diagnostic ignored "-Wrestrict"
    ...
    #define ECS_EXTERN_C extern "C"
#else
    #define ECS_EXTERN_C extern
#endif
...
#if !defined(FLECS_META_C_EXPORT) || !defined(FLECS_META_C_IMPORT)
#ifndef flecs_STATIC
...
FLECS_API
void ecs_vec_set_stb_length(
    ecs_vec_t* v,
    int32_t count);

FLECS_API
void ecs_vec_add_stb_length(
    ecs_vec_t* v,
    int32_t add_count);
...
#define ecs_map_count(map) ((map) ? (int32_t)hmlenu((map)->stb_hm) : 0)
...
#define flecs_hashmap_ensure(map, key, V)\
    flecs_hashmap_ensure_(map, ECS_SIZEOF(*key), ECS_ALIGNOF(*key), key, ECS_SIZEOF(V), ECS_ALIGNOF(V))
...
#define flecs_hashmap_set(map, key, value)\
    flecs_hashmap_set_(map, ECS_SIZEOF(*key), ECS_ALIGNOF(*key), key, ECS_SIZEOF(*value), ECS_ALIGNOF(*value), value)
...
#define ECS_COMPONENT_DEFINE(world, id_) \
    {\
        ecs_assert(ecs_lookup_fullpath(world, #id_) == 0, ECS_ID_IN_USE, "component with this name already exists"); \
...
#define ECS_OBSERVER_DEFINE(world, id_, kind, ...)\
    {\
        ecs_assert(ecs_lookup_fullpath(world, #id_) == 0, ECS_ID_IN_USE, "observer with this name already exists"); \
...
#define ecs_set(world, entity, component, ...)\
    ecs_set_id(world, entity, ecs_id(component), sizeof(component), &__VA_ARGS__)
#else
#define ecs_set(world, entity, component, ...)\
...
#define ECS_SYSTEM_DEFINE(world, id_, phase, ...) \
    { \
        ecs_assert(ecs_lookup_fullpath(world, #id_) == 0, ECS_ID_IN_USE, "system with this name already exists"); \
...
/* Begin The-Forge changes */
/**
 *  Use to declare globals for array sizes
 */
#define ECS_I64_CONSTANT_DEFINE(world, id)                                              \
    {                                                                                   \
        ecs_entity_desc_t desc = {};                                                    \
        desc.name = #id;                                                                \
        ecs_assert(!isdigit(desc.name[0]), ECS_INVALID_PARAMETER,                       \
            "Macros are not supported for constants");                                  \
        ecs_id(id) = ecs_entity_init(world, &desc);                                     \
        ecs_assert(id >= INT64_MIN && id <= INT64_MAX, ECS_INVALID_PARAMETER, NULL);    \
        const ecs_i64_t value = (ecs_i64_t)id;                                          \
        ecs_set_id(world, ecs_id(id), ecs_id(ecs_i64_t), sizeof(ecs_i64_t), &value);    \
    }                                                                                   \
 ...
 *  Use to declare globals for array sizes
 */
#define ECS_I64_CONSTANT(world, id)     \
    ecs_entity_t ecs_id(id) = 0;        \
    ECS_I64_CONSTANT_DEFINE(world, id); \
    (void)ecs_id(id)
/* End The-Forge changes */
...
#define ECS_STRUCT_IMPL(name, type_desc)\
    ECS_EXTERN_C FLECS_META_C_EXPORT ECS_COMPONENT_DECLARE(name);\
    static const char *FLECS__##name##_desc = type_desc;\
    static ecs_type_kind_t FLECS__##name##_kind = EcsStructType;\
    ECS_EXTERN_C FLECS_META_C_EXPORT ECS_COMPONENT_DECLARE(name) = 0
...
#define ECS_STRUCT_EXTERN(name, type_desc)\
    ECS_EXTERN_C FLECS_META_C_IMPORT ECS_COMPONENT_DECLARE(name)
