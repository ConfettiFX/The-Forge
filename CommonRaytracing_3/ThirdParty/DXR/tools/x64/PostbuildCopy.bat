...
set toolsDirectory=%2
echo %1
copy %toolsDirectory%dxcompiler.dll %outputDirectory%
copy %toolsDirectory%dxcompiler.pdb %outputDirectory%
copy %toolsDirectory%dxil.dll %outputDirectory%

if %UseRS4Binaries% NEQ 0 ( 
	echo RS4-DXR compatible build detected, copying DXR D3D12 binaries
   copy %toolsDirectory%d3d12.dll %outputDirectory%
   copy %toolsDirectory%dxgidebug.dll %outputDirectory%
   copy %toolsDirectory%d3d12sdklayers.dll %outputDirectory%
)
